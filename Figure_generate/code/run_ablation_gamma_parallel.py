#!/usr/bin/env python
"""
Ablation Study: Gamma Distribution with CV = 1/sqrt(3) - PARALLELIZED
...
Usage:
python run_ablation_gamma_parallel.py
python run_ablation_gamma_parallel.py --n_cores 10
"""
...
def run_single_repetition(args):
    ...
    seed = int(u * 10000) + rep * 100000 + 2000000  # Offset for gamma
    np.random.seed(seed)

    # Initialize interaction matrix with Gamma distribution
    I = np.zeros((N, N))
    for i in range(N):
        for j in range(N):
            I[i, j] = gamma_distribution(u)
    for i in range(N):
        I[i, i] = 1
...
def simulate_parallel(n_cores=None, n_reps=400, u_step=0.02):
    """Run the Gamma ablation simulation with parallelization."""
    ...
    session_name = f"Simulation_Data/48species_ablation_gamma_superfine"
    ...
    print(f"\nGAMMA ABLATION - PARALLELIZED SUPERFINE:")
    print(f"=" * 60)
    ...
    output_file = f"{session_name}/Community_ablation_gamma_superfine.json"
    ...
    params_dict = {
        ...
        'distribution_type': 'gamma', 'CV': 1/np.sqrt(3), 'n_cores': n_cores
    }
    ...
    for i, u in enumerate(u_list):
        ...
        args_list = [(u, rep, N, num_S, num_C, t, threshold, session_name) for rep in range(n_reps)]
    ...
    print(f"\n{'='*70}")
    print(f"GAMMA ABLATION COMPLETED! Total time: {total_time_hours:.2f} hours")
    print(f"Data saved to: {output_file}")
    print(f"{'='*70}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run parallel Gamma ablation simulations')
