#!/usr/bin/env python
"""
Ablation Study: Growth Rate Heterogeneity with std=0.1 - PARALLELIZED
...
Usage:
python run_ablation_growth_rate_std01_parallel.py
python run_ablation_growth_rate_std01_parallel.py --n_cores 10
"""
...
def run_single_repetition(args):
    ...
    seed = int(u * 10000) + rep * 100000 + 3000000  # Offset for growth rate std01
    np.random.seed(seed)
...
def simulate_parallel(n_cores=None, n_reps=400, u_step=0.02, growth_std=0.1):
    ...
    session_name = f"Simulation_Data/48species_ablation_growth_std01_superfine"
    ...
    output_file = f"{session_name}/Community_ablation_growth_std01_superfine.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run parallel Growth Rate ablation simulations (std=0.1)')
    ...
    simulate_parallel(n_cores=args.n_cores, n_reps=args.n_reps, u_step=args.u_step, growth_std=0.1)
