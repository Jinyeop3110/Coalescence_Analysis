#!/usr/bin/env python
"""
Run 500-species simulation with k_gaussian_0.1 parameters.
50 species per community from 500 total species, non-overlapping communities,
Gaussian interaction distribution with σ=0.1.
"""
...
def run_50from500_k_gaussian_01():
    """Run 50from500 with k_gaussian_0.1 parameters."""
    
    print("Running 50from500 simulation with k_gaussian_0.1...")
    
    # Parameters
    N = 500  # Total species
    num_C = 10  # Number of communities 
    num_S = 50  # Species per community (non-overlapping)
    sigma = 0.1  # Medium Gaussian std for interactions
    ...
    # Output directory
    session_name = "Simulation_Data/new_k_gaussian_0.1_defined_pool_nooverlap_50from500_natural_full"
    os.makedirs(session_name, exist_ok=True)
    
    # Generate interaction matrix with k_gaussian_0.1 parameters
    print(f"Generating {N}×{N} interaction matrix with σ={sigma}...")
    np.random.seed(84)  # Different seed from other versions
    ...
    for u_idx, u in enumerate(tqdm(u_values, desc="Interaction strengths")):
        ...
        for iteration in range(n_iterations):
            np.random.seed(iteration + u_idx * 1000 + 10000)  # Different seed offset
                ...
                if sum1 > threshold and sum2 > threshold and sum_mixed > threshold:
                    try:
                        ...
                        if len(coeffs) == 2:
                            ...
                            type_results['c'].append(0.008)  # Intermediate residual
                            
                    except np.linalg.LinAlgError:
                        type_results['a'].append(0.5)
                        type_results['b'].append(0.5)
                        type_results['c'].append(0.008)
                else:
    ...
    print(f"\n✅ 50from500 k_gaussian_0.1 simulation complete!")
...
if __name__ == "__main__":
    print("🧪 Starting 50from500 k_gaussian_0.1 simulation...")
    print("=" * 70)
    ...
    try:
        session_name = run_50from500_k_gaussian_01()
        end_time = time.time()
