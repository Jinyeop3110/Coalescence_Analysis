#!/usr/bin/env python
"""
Ablation Study: Carrying Capacity Heterogeneity with std=0.1
...
- Carrying capacity: N(1, 0.1^2), clipped to positive values
  - Mean = 1
  - Std = 0.1
  - CV = 0.1
...
python run_ablation_carrying_capacity_std01.py
"""
...
def carrying_capacity_gaussian_std01():
    """
    Generate Gaussian random carrying capacity with mean=1, std=0.1.
    Clips to minimum of 0.01 to avoid division by zero.
    """
    k = np.random.normal(1.0, 0.1)
    return max(0.01, k)
...
def simulate_ablation_carrying_capacity():
    """Run the ablation study with carrying capacity heterogeneity (std=0.1)."""

    session_name = "Simulation_Data/48species_ablation_k_std01"
    ...
    print(f"\n" + "="*70)
    print("ABLATION STUDY: CARRYING CAPACITY HETEROGENEITY (std=0.1)")
    print("="*70)
    print(f"Carrying capacity distribution: N(1, 0.1^2), clipped to >0.01")
    ...
    output_file = session_name + '/Community_ablation_k_std01.json'
    ...
    params_dict = {
        ...
        't': t,
        'threshold': threshold,
        'carrying_capacity_distribution': 'N(1, 0.1^2)',
        'carrying_capacity_mean': 1.0,
        'carrying_capacity_std': 0.1,
    ...
    for i, mu in enumerate(mu_list):
        ...
        for rep in range(N_reps):
            ...
            seed = int(mu * 10000) + rep * 100000 + 4000000  # Different seed offset from growth rate
            np.random.seed(seed)

            # Initialize species pool with heterogeneous carrying capacities
            I, g, k = InitializeSpeceiesPool(N,
                                           lambda: uniform_distribution(mu),
                                           f_g=lambda: 1,  # Constant growth rate
                                           f_k=carrying_capacity_gaussian_std01,  # Heterogeneous K
    ...
    print(f"\n{'='*70}")
    print("CARRYING CAPACITY ABLATION (std=0.1) SIMULATION COMPLETED!")
    print(f"{'='*70}")
    ...
    print(f"Use: python plot_phase_diagram_ablation_carrying_capacity.py")


if __name__ == "__main__":
    simulate_ablation_carrying_capacity()
