#!/usr/bin/env python
"""
Plot Phase Diagrams for Growth Rate Ablation Studies

This script creates phase diagrams from the growth rate ablation study simulation data.
- std=0.1: N(1, 0.1^2) growth rate distribution
- std=0.2: N(1, 0.2^2) growth rate distribution

Inputs:
- Simulation_Data/48species_ablation_growth_std01/Community_ablation_growth_std01.json
- Simulation_Data/48species_ablation_growth_std02/Community_ablation_growth_std02.json

Outputs:
- Figure/PhaseDiagram_ablation/Fig_phase_diagram_ablation_growth_std01.svg/png/pdf
- Figure/PhaseDiagram_ablation/Fig_phase_diagram_ablation_growth_std02.svg/png/pdf

Usage:
python plot_phase_diagram_ablation_growth_rate.py
"""
...
def main():
    ...
    ablation_sessions = [
        {
            "name": "growth_std01",
            "json_file": "Simulation_Data/48species_ablation_growth_std01/Community_ablation_growth_std01.json",
            "output_base": "Figure/PhaseDiagram_ablation/Fig_phase_diagram_ablation_growth_std01",
            "description": "Growth rate N(1, 0.1^2)"
        },
        {
            "name": "growth_std02",
            "json_file": "Simulation_Data/48species_ablation_growth_std02/Community_ablation_growth_std02.json",
            "output_base": "Figure/PhaseDiagram_ablation/Fig_phase_diagram_ablation_growth_std02",
    ...
    for session in ablation_sessions:
        ...
        if not os.path.exists(session['json_file']):
            ...
            print(f"  Run simulation first: python run_ablation_{session['name']}.py", flush=True)
            continue
