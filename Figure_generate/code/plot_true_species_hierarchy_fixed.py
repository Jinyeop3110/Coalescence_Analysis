#!/usr/bin/env python3
"""
plot_true_species_hierarchy_fixed.py

Purpose: Calculates and plots TRUE species-to-species (ASV-to-ASV) dominance hierarchy
WITH FIXED ASV ORDERING BY INDEX
Key features:
- Creates ASV dominance matrices from coalescence abundance data
- ASVs are always ordered by their index (ASV1, ASV2, ... ASV12)
- Fixed to use only valid ASVs (1-43)
...
def load_data():
    ...
    # Verify ASV naming
    print("ASV columns sample:", asv_columns[:5], "...", asv_columns[-5:])
...
def get_dominant_asvs(processed_sequences, sample_ids, species_pool_num=12, min_abundance=0.05):
    """
    Get the most abundant ASVs for each sample.
    For species_pool_num experiments, we expect to find ASVs 1-12
    Returns dict: {sample_id: [list of dominant ASV names]}
    """
    asv_columns = [col for col in processed_sequences.columns if col != 'SampleIDX']
    sample_dominant_asvs = {}
    
    # Track which ASVs appear across all samples
    all_appearing_asvs = set()
    ...
    for sample_id in sample_ids:
        ...
        # Store all ASVs that appear as dominant
        sample_dominant_asvs[sample_id] = abundant_asvs.index.tolist()
        all_appearing_asvs.update(abundant_asvs.index.tolist())
    
    print(f"ASVs appearing as dominant: {sorted(all_appearing_asvs)}")
    
    return sample_dominant_asvs, sorted(list(all_appearing_asvs))

def extract_asv_number(asv_name):
    """Extract the numeric part from ASV name like 'NormalizedAbundance12' -> 12"""
    try:
        return int(asv_name.replace('NormalizedAbundance', ''))
    except:
        return 999  # Return high number for any parsing errors
...
def calculate_asv_dominance_from_coalescence(Coalescence_data, processed_sequences, 
    ...
    parent_dominant_asvs, all_dominant_asvs = get_dominant_asvs(
        processed_sequences, Sub_IDX_list, species_pool_num=species_pool_num, min_abundance=0.1)
    
    # Filter to only include ASVs 1-species_pool_num
    filtered_asvs = []
    for asv in all_dominant_asvs:
        asv_num = extract_asv_number(asv)
        if 1 <= asv_num <= species_pool_num:
            filtered_asvs.append(asv)
        else:
            print(f"WARNING: Found unexpected ASV: {asv} (number: {asv_num})")
    
    # Sort ASVs by their numeric index
    all_dominant_asvs = sorted(filtered_asvs, key=extract_asv_number)
    
    print(f"Found {len(all_dominant_asvs)} valid dominant ASVs for {species_pool_num}-species pool")
    print(f"ASVs included: {[f'ASV{extract_asv_number(asv)}' for asv in all_dominant_asvs]}")
    ...
    for SampleIDX in Coal_IDX_list:
        ...
        parent1_asvs = [asv for asv in parent_dominant_asvs[subSampleIDX1] 
                       if asv in all_dominant_asvs]
        parent2_asvs = [asv for asv in parent_dominant_asvs[subSampleIDX2] 
                       if asv in all_dominant_asvs]
...
def plot_asv_hierarchy_with_significance(Coalescence_data, processed_sequences, Metadata, exception_list,
    ...
    """Plot ASV hierarchy with significance testing - FIXED VERSION WITH INDEX ORDERING"""
    ...
    asv_matrix, dominant_asvs = calculate_asv_dominance_from_coalescence(
        Coalescence_data, processed_sequences, Coal_IDX_list, Sub_IDX_list, Variable2plot, species_num)
    
    if len(dominant_asvs) < 2:
        print("Not enough dominant ASVs found for analysis")
        return np.nan, np.nan
    
    # Convert to DataFrame - ASVs are already sorted by index
    ...
    # IMPORTANT: Do NOT sort by competitive ability - keep ASVs in index order
    df_sorted = df  # Keep original index order
    ...
    axs[0].set_xlabel('ASVs (ordered by index)')
    axs[0].set_ylabel('ASVs (ordered by index)')
    
    # Simplify ASV labels for readability
    simplified_labels = [f"ASV{extract_asv_number(col)}" for col in df_sorted.columns]
    ...
    # Plot competitive scores (still showing competitive ability but keep x-axis in index order)
    row_means = df.mean(axis=1, skipna=True)
    competitive_scores = row_means.values
    axs[2].bar(range(len(competitive_scores)), competitive_scores)
    axs[2].set_xlabel('ASVs (ordered by index)')
    axs[2].set_ylabel('Mean Dominance Score')
    axs[2].set_title('ASV Competitive Ability')
    ...
    # Save figure
    filename = f"TRUE_ASV_Hierarchy_FIXED_{Variable2plot}_{com_type}_{medium}_{species_num}_rep{rep}.png"
...
def main():
    ...
    print("Starting TRUE ASV (Species) Hierarchy Analysis - FIXED VERSION...")
    ...
    if results:
        ...
        results_df.to_csv(os.path.join(output_dir, "TRUE_ASV_hierarchy_results_FIXED.csv"), index=False)
        print("\nTRUE ASV Hierarchy Results (FIXED):")
