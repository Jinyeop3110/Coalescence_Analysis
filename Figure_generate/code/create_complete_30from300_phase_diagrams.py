#!/usr/bin/env python
"""
Create phase diagrams for complete 30from300 k_gaussian simulations.
Uses full u-range data (0.1-1.2) matching 50from500 system.
"""
...
def classify_vector_decomposition_k_gaussian_30from300(a, b, c, sigma):
    ...
    if sigma <= 0.05:
        # Very tight distribution - more sensitive thresholds
        c_threshold = 0.05
        diff_threshold = 0.15
    elif sigma <= 0.1:
        # Medium distribution
...
def create_30from300_phase_diagram_complete(sigma, data_path, output_path):
    """Create phase diagram for complete 30from300 k_gaussian simulation."""
    
    print(f"📊 Creating complete 30from300 k_gaussian_{sigma} phase diagram...")
    ...
    try:
        ...
        print(f"Full range: {u_values[0]:.1f} to {u_values[-1]:.1f} ({len(u_values)} points)")
 ...
 <metadata>
  <rdf:RDF xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
   <cc:Work>
    ...
    <dc:creator>
     <cc:Agent>
      <dc:title>30from300 k_gaussian_{sigma} phase diagram (complete)</dc:title>
     </cc:Agent>
    </dc:creator>
   </cc:Work>
  </rdf:RDF>
...
\" style=\"fill: none; stroke: #262626; stroke-width: 0.5; stroke-linejoin: miter; stroke-linecap: square\"/>
        ...
        print(f"✅ Complete phase diagram created: {output_file}")
        print(f"   30from300 k_gaussian_{sigma} with full u-range ({len(u_values)} points)")
...
def main():
    """Create complete phase diagrams for all three 30from300 k_gaussian simulations."""
    
    print("📊 Creating complete 30from300 phase diagrams")
    print("Full interaction range (0.1-1.2) matching 50from500 system")
    print("=" * 70)
    
    # Configuration for complete simulations
    simulations = [
        {
            'sigma': 0.05,
            'data_path': 'Simulation_Data/complete_k_gaussian_0.05_defined_pool_nooverlap_30from300_natural_full/Similarity.xlsx',
            'output_path': 'Figure/PhaseDiagram/Fig_phase_diagram_Simul_30from300_k_gaussian_0.05_complete.svg'
        },
        {
            'sigma': 0.1,
            'data_path': 'Simulation_Data/complete_k_gaussian_0.1_defined_pool_nooverlap_30from300_natural_full/Similarity.xlsx',
            'output_path': 'Figure/PhaseDiagram/Fig_phase_diagram_Simul_30from300_k_gaussian_0.1_complete.svg'
        },
        {
            'sigma': 0.15,
            'data_path': 'Simulation_Data/complete_k_gaussian_0.15_defined_pool_nooverlap_30from300_natural_full/Similarity.xlsx',
            'output_path': 'Figure/PhaseDiagram/Fig_phase_diagram_Simul_30from300_k_gaussian_0.15_complete.svg'
        }
    ]
    ...
    for sim_config in simulations:
        ...
        if create_30from300_phase_diagram_complete(
            sim_config['sigma'],
    ...
    print(f"\n🎉 Complete phase diagram generation finished!")
    ...
    if success_count > 0:
        ...
        for sim_config in simulations:
            ...
            if output_path.exists():
                ...
                print(f"  • k_gaussian_{sim_config['sigma']} → {output_path.name} ({file_size:,} bytes)")
    
    print("\n🔗 All 30from300 phase diagrams now match 50from500 u-range (0.1-1.2)")
    print("=" * 70)
