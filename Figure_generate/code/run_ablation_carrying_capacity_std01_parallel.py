#!/usr/bin/env python
"""
Ablation Study: Carrying Capacity Heterogeneity with std=0.1 - PARALLELIZED
...
- Carrying capacity: N(1, 0.1^2), clipped to >0.01

Usage:
python run_ablation_carrying_capacity_std01_parallel.py
python run_ablation_carrying_capacity_std01_parallel.py --n_cores 10
"""
...
def run_single_repetition(args):
    ...
    seed = int(u * 10000) + rep * 100000 + 4000000  # Offset for carrying capacity std01
    np.random.seed(seed)
...
def simulate_parallel(n_cores=None, n_reps=400, u_step=0.02, k_std=0.1):
    ...
    session_name = f"Simulation_Data/48species_ablation_k_std01_superfine"
    ...
    output_file = f"{session_name}/Community_ablation_k_std01_superfine.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run parallel Carrying Capacity ablation simulations (std=0.1)')
    ...
    simulate_parallel(n_cores=args.n_cores, n_reps=args.n_reps, u_step=args.u_step, k_std=0.1)
