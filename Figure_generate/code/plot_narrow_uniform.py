...
def plot_phase_diagrams_with_matrices():
    ...
    json_sessions = [
        {
            "name": "10reps_narrow_uniform",
            "json_file": "Simulation_Data/48species_10reps_narrow_uniform/Community_10reps_narrow_uniform.json",
            "description": "10 reps × 24 intensities (0.05-1.2) with narrow uniform [0.5μ,1.5μ] CV≈0.289",
            "output_suffix": "10reps_narrow_uniform"
        }
    ]
...
def main():
    ...
    # Check narrow uniform data
    json_narrow = "Simulation_Data/48species_10reps_narrow_uniform/Community_10reps_narrow_uniform.json"
    if Path(json_narrow).exists():
        try:
            with open(json_narrow, 'r') as f:
                data_narrow = json.load(f)
            total_narrow = sum(len(data_narrow[u_key]) for u_key in data_narrow.keys())
            print(f"  ✓ Narrow uniform data: {total_narrow} simulations available")
        except:
            print(f"  ⚠️  Narrow uniform data: File exists but cannot read")
    else:
        print(f"  ✗ Narrow uniform data: Not found")
