#!/usr/bin/env python
"""
Create 500-species phase diagram matching the exact matplotlib SVG style.
Uses data from the existing analysis and creates SVG that matches Fig_phase_diagram_Simul_k_gaussian_0.25.svg exactly.
"""
...
def classify_vector_decomposition_corrected(a, b, c):
    """
    Classify coalescence outcome based on vector decomposition coefficients.
    Uses corrected thresholds that match the standard simulation pipeline.
    """
    ...
    # Use the same classification logic as common_setup_simulation_fix.py
    if c_norm > 0.5:
...
def process_500species_data_corrected():
    """Process 500-species data with corrected classification to match standard pipeline."""
    
    print("Processing 500-species data with corrected classification...")
    ...
    for u_val in u_values:
        ...
        if col_name in data1.columns:
            ...
            if min_length > 0:
                # Classify each data point using corrected classification
                classifications = [classify_vector_decomposition_corrected(a_values[j], b_values[j], c_values[j]) 
...
def create_matplotlib_svg():
    """Create SVG that exactly matches matplotlib style from existing simulations."""
    
    print("Creating matplotlib-style 500-species phase diagram...")
    
    # Process the data with corrected classification
    fractions_data = process_500species_data_corrected()
    if fractions_data is None:
        print("Failed to process data")
        return
    
    # Exact matplotlib dimensions from reference file
    width_pt = 177.58425
    height_pt = 158.917775
    
    # Plot area coordinates (from Fig_phase_diagram_Simul_k_gaussian_0.25.svg)
    plot_left = 33.13625
    ...
    # Standard simulation colors (from COLORMAP.py - lighter versions)
    colors = {
    ...
    # Create SVG with exact matplotlib structure
 ...
 <metadata>
  <rdf:RDF xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
   <cc:Work>
    ...
    <dc:creator>
     <cc:Agent>
      <dc:title>500-species phase diagram (standard pipeline style)</dc:title>
     </cc:Agent>
    </dc:creator>
   </cc:Work>
  </rdf:RDF>
...
" style="fill: none; stroke: #262626; stroke-width: 0.5; stroke-linejoin: miter; stroke-linecap: square"/>
    ...
    print(f"   Using corrected classification thresholds")
