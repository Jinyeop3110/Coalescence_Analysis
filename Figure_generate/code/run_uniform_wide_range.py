"""
Run 48-species coalescence simulations with WIDE uniform distribution
Uniform[0, 2*mean] - the standard wide range

This gives:
- Mean: equals 'mean' parameter
- Range: [0, 2*mean] (wide range)
- Variance: (mean)^2 / 3
- CV: 1/sqrt(3) ≈ 0.577 (standard uniform CV)
"""
...
def uniform_wide_range(mean):
    """
    Sample from Uniform[0, 2*mean]

    Properties:
    - Mean: mean
    - Variance: (mean)^2 / 3
    - Std: mean / sqrt(3) ≈ 0.577 * mean
    - CV: 1/sqrt(3) ≈ 0.577 (constant!)
    - Range: [0, 2*mean]
    """
    return 2.0 * mean * np.random.random()  # Uniform[0, 2*mean]
...
def run_single_simulation(u, rep, seed):
    ...
    # Fill off-diagonal elements with wide uniform distribution
    for i in range(N):
        for j in range(N):
            if i != j:
                I[i, j] = uniform_wide_range(u)
...
def main():
    print("="*70)
    print("48-Species Coalescence Simulation with Wide Uniform Distribution")
    print("Distribution: Uniform[0, 2*mean]")
    print("="*70)

    # Calculate theoretical CV
    theoretical_cv = 1.0 / np.sqrt(3)
    print(f"\nTheoretical CV: {theoretical_cv:.4f} (≈0.577)")
    print(f"Compare to narrow uniform [0.5*mean, 1.5*mean]: CV ≈ 0.289")
    print(f"This has FOUR TIMES the variance!\n")
    ...
    output_dir = "Simulation_Data/48species_20reps_wide_uniform"
    ...
    output_file = os.path.join(output_dir, "Community_20reps_wide_uniform.json")
