#!/usr/bin/env python
"""
Ablation Study: Growth Rate Heterogeneity with std=0.2 - PARALLELIZED
...
- Intrinsic growth rate: N(1, 0.2^2), clipped to 0 if negative

Usage:
python run_ablation_growth_rate_std02_parallel.py
python run_ablation_growth_rate_std02_parallel.py --n_cores 10
"""
...
def run_single_repetition(args):
    ...
    u, rep, N, num_S, num_C, t, threshold, growth_std, session_name = args

    seed = int(u * 10000) + rep * 100000 + 3500000  # Offset for growth rate std02
    np.random.seed(seed)
    ...
    # Heterogeneous growth rates: N(1, growth_std^2), clipped to 0 if negative
    g = np.random.normal(1.0, growth_std, N)
    g = np.maximum(g, 0.0)
...
def simulate_parallel(n_cores=None, n_reps=400, u_step=0.02, growth_std=0.2):
    """Run the Growth Rate ablation simulation with parallelization."""
    ...
    session_name = f"Simulation_Data/48species_ablation_growth_std02_superfine"
    ...
    print(f"\nGROWTH RATE ABLATION (std={growth_std}) - PARALLELIZED SUPERFINE:")
    print(f"=" * 60)
    print(f"Growth rate distribution: N(1, {growth_std}^2)")
    ...
    output_file = f"{session_name}/Community_ablation_growth_std02_superfine.json"
    ...
    params_dict = {
        ...
        'growth_rate_distribution': f'N(1, {growth_std}^2)',
        'growth_rate_std': growth_std,
    ...
    for i, u in enumerate(u_list):
        ...
        args_list = [(u, rep, N, num_S, num_C, t, threshold, growth_std, session_name) for rep in range(n_reps)]
    ...
    print(f"\n{'='*70}")
    print(f"GROWTH RATE ABLATION (std={growth_std}) COMPLETED! Total time: {total_time_hours:.2f} hours")
    print(f"Data saved to: {output_file}")
    print(f"{'='*70}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run parallel Growth Rate ablation simulations (std=0.2)')
    ...
    simulate_parallel(n_cores=args.n_cores, n_reps=args.n_reps, u_step=args.u_step, growth_std=0.2)
