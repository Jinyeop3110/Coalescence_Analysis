#!/usr/bin/env python
"""
Ablation Study: Growth Rate Heterogeneity with std=0.2
...
python run_ablation_growth_rate_std02.py
"""
...
def growth_rate_gaussian_std02():
    """
    Generate Gaussian random growth rate with mean=1, std=0.2.
    Clips negative values to 0.
    """
    g = np.random.normal(1.0, 0.2)
    return max(0.0, g)
...
def simulate_ablation_growth_rate():
    """Run the ablation study with growth rate heterogeneity (std=0.2)."""

    session_name = "Simulation_Data/48species_ablation_growth_std02"
    ...
    print(f"\n" + "="*70)
    print("ABLATION STUDY: GROWTH RATE HETEROGENEITY (std=0.2)")
    print("="*70)
    print(f"Growth rate distribution: N(1, 0.2^2), clipped to 0 if negative")
    ...
    output_file = session_name + '/Community_ablation_growth_std02.json'
    ...
    params_dict = {
        ...
        't': t,
        'threshold': threshold,
        'growth_rate_distribution': 'N(1, 0.2^2)',
    ...
    print(f"\n{'='*70}")
    print("GROWTH RATE ABLATION (std=0.2) SIMULATION COMPLETED!")
    print(f"{'='*70}")
