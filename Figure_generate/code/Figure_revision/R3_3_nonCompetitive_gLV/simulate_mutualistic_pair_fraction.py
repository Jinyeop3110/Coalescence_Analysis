#!/usr/bin/env python
"""
R3-4 mutualistic-pair fraction sweep.

This simulation is designed for the reviewer's question about whether a
mutualistic pair should be considered weak or strong. For a controlled fraction
q of unordered species pairs, both directions are made weakly facilitative:

    alpha_ij, alpha_ji ~ U[-f_mutual * mu, 0]

where alpha < 0 is facilitation under the manuscript sign convention. All
remaining off-diagonal coefficients are drawn from the baseline competitive
distribution U[0, 2mu]. Dynamics use the same density-dependent self-limitation
as the mixed-sign R3-4 robustness analysis:

    dn_i/dt = n_i * (1 - (A n)_i - gamma * n_i^2)

Outputs:
  mutualistic_pair_fraction_results.json
"""
...
ODE_METHOD = 'LSODA'
OUT_PATH = os.path.join(HERE, 'mutualistic_pair_fraction_results.json')
...
def sample_I_mutualistic_pairs(mu, mutual_pair_fraction, rng):
    """Draw matrix with a controlled fraction of bidirectional weak mutualisms."""
    I = rng.uniform(0.0, 2.0 * mu, size=(N_POOL, N_POOL))
    iu, ju = np.triu_indices(N_POOL, k=1)
    n_pairs = len(iu)
    n_mutual = int(round(mutual_pair_fraction * n_pairs))

    if n_mutual > 0:
        chosen = rng.choice(n_pairs, size=n_mutual, replace=False)
        ii = iu[chosen]
        jj = ju[chosen]
        I[ii, jj] = rng.uniform(-F_MUTUAL * mu, 0.0, size=n_mutual)
        I[jj, ii] = rng.uniform(-F_MUTUAL * mu, 0.0, size=n_mutual)

    np.fill_diagonal(I, 1.0)
    return I, n_mutual / n_pairs


def assemble_pool(mu, mutual_pair_fraction, rng, gamma=GAMMA):
    for attempt in range(MAX_RESAMPLE):
        I, realized_fraction = sample_I_mutualistic_pairs(
            mu, mutual_pair_fraction, rng)
        ...
        return I, comms, sc, attempt, realized_fraction
...
def run():
        ...
        with open(OUT_PATH, 'w') as fh:
            json.dump({
                'parameters': {
                    ...
                    'N_POOLS': N_POOLS,
                    'MU_VALUES': MU_VALUES,
                    'MUTUAL_PAIR_FRACTIONS': MUTUAL_PAIR_FRACTIONS,
                    'F_MUTUAL': F_MUTUAL,
                    'GAMMA': GAMMA,
    ...
    for mu in MU_VALUES:
        for mutual_fraction in MUTUAL_PAIR_FRACTIONS:
            key = f'mu={mu:.2f}_q={mutual_fraction:.2f}'
            print(f'\n[{key}] f_mutual={F_MUTUAL:.2f}, gamma={GAMMA:.2f} -- '
            ...
            for pool_idx in range(N_POOLS):
                ...
                out = assemble_pool(mu, mutual_fraction, sub_rng, gamma=GAMMA)
                if out is None:
                    rejects += MAX_RESAMPLE
                    continue
                I, comms, sc, attempts, realized_fraction = out
                rejects += attempts
                realized_fractions.append(realized_fraction)
            ...
            results[key] = {
                'mu': mu,
                'mutual_pair_fraction_target': mutual_fraction,
                'mutual_pair_fraction_realized': (
                    float(np.mean(realized_fractions))
                    if realized_fractions else float('nan')),
                'f_mutual': F_MUTUAL,
                'gamma': GAMMA,
