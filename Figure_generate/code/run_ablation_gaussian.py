#!/usr/bin/env python
"""
Ablation Study: Gaussian Distribution with CV = 1/sqrt(3)
...
python run_ablation_gaussian.py
"""
...
def gaussian_distribution(mu):
    ...
    Returns:
    Random sample from N(mu, (mu/sqrt(3))^2)
    """
    if mu <= 0:
        return 0.0
    std = mu / np.sqrt(3)
...
def simulate_ablation_gaussian():
    """Run the ablation study with Gaussian distribution."""

    session_name = "Simulation_Data/48species_ablation_gaussian"
    ...
    print(f"\n" + "="*70)
    print("ABLATION STUDY: GAUSSIAN DISTRIBUTION")
    print("="*70)
    ...
    output_file = session_name + '/Community_ablation_gaussian.json'
    ...
    for i, mu in enumerate(mu_list):
        ...
        print(f"  Gaussian: N({mu:.2f}, {mu/np.sqrt(3):.4f}^2)")
        print(f"{'='*70}")
        ...
        for rep in range(N_reps):
            ...
            seed = int(mu * 10000) + rep * 100000 + 1000000  # Offset to differ from uniform
            np.random.seed(seed)

            # Initialize species pool with Gaussian distribution
            I, g, k = InitializeSpeceiesPool(N,
                                           lambda: gaussian_distribution(mu),
    ...
    print(f"\n{'='*70}")
    print("GAUSSIAN ABLATION SIMULATION COMPLETED!")
    print(f"{'='*70}")
