#!/usr/bin/env python3
"""
plot_true_species_hierarchy_all_asvs.py

Purpose: Calculates and plots TRUE species-to-species (ASV-to-ASV) dominance hierarchy
WITH ALL DOMINANT ASVs INCLUDED AND ORDERED BY INDEX
Key features:
- Creates ASV dominance matrices from coalescence abundance data
- ASVs are ordered by their index (ASV1, ASV2, ... ASV43)
- Includes ALL dominant ASVs (no filtering by species pool number)
...
def get_dominant_asvs(processed_sequences, sample_ids, min_abundance=0.05):
    """
...
def calculate_asv_dominance_from_coalescence(Coalescence_data, processed_sequences, 
    ...
    Now includes ALL dominant ASVs regardless of number.
    """
    ...
    parent_dominant_asvs, all_dominant_asvs = get_dominant_asvs(
        processed_sequences, Sub_IDX_list, min_abundance=0.1)
    
    # Sort ASVs by their numeric index
    all_dominant_asvs = sorted(all_dominant_asvs, key=extract_asv_number)
...
def plot_asv_hierarchy_with_significance(Coalescence_data, processed_sequences, Metadata, exception_list,
    ...
    """Plot ASV hierarchy with significance testing - ALL ASVs VERSION"""
    ...
    # IMPORTANT: Keep ASVs in index order (no sorting by competitive ability)
    ...
    # Save figure
    filename = f"TRUE_ASV_Hierarchy_ALL_{Variable2plot}_{com_type}_{medium}_{species_num}_rep{rep}.png"
...
def main():
    ...
    print("Starting TRUE ASV (Species) Hierarchy Analysis - ALL ASVs VERSION...")
    ...
    if results:
        ...
        results_df.to_csv(os.path.join(output_dir, "TRUE_ASV_hierarchy_results_ALL.csv"), index=False)
        print("\nTRUE ASV Hierarchy Results (ALL ASVs):")
