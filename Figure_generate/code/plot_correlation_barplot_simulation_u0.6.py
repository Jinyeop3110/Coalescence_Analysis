#!/usr/bin/env python3
"""
Create barplot for simulation data at u=0.6 (matching experimental barplot style).
"""
...
def plot_barplot_u06(results, save_dir):
    """
    Create bar plot for u=0.6 simulation data.
    Style matching the experimental data barplots.
    """
    ...
    # Use stratified sampling to better represent the distribution
    # Divide data into quantiles and sample proportionally from each
    n_display = 50

    # For same-origin correlations
    same_corrs_array = np.array(same_origin_corrs)
    same_sorted_indices = np.argsort(same_corrs_array)
    n_bins = 10  # Divide into 10 bins (deciles)
    ...
    ax.axhline(y=null_mean, color='#95a5a6', linestyle='-',
               linewidth=2, alpha=0.7, label='Random Selection', zorder=5)
    ...
    # Make y-axis ticks at 0.1 intervals (matching experimental plots)
    ...
    # Use dynamic y-limits like experimental plots
    if stars_diff != 'ns':
    ...
    save_path_png = os.path.join(save_dir, 'correlation_barplot_simulation_u0.6.png')
    plt.savefig(save_path_png, dpi=300, bbox_inches='tight')
    print(f"Saved: {save_path_png}")

    save_path_svg = os.path.join(save_dir, 'correlation_barplot_simulation_u0.6.svg')
...
def main():
    """Main function."""
    print("="*80)
    print("CREATING BARPLOT FOR SIMULATION u=0.6")
    print("="*80)
    ...
    results = analyze_simulation_per_event_by_pair_type(
        data, 0.60, threshold=1e-4, n_simulations=1000
    )
    ...
    plot_barplot_u06(results, save_dir)

    # Print summary
    print("\n" + "="*80)
    print("SUMMARY:")
    print("="*80)
    print(f"n_events: {results['n_events']}")
    print(f"Same Parent: {results['summary']['mean_corr_same']:.4f}")
    print(f"Cross Parents: {results['summary']['mean_corr_mixed']:.4f}")
    print(f"Random: {np.mean([results['null_results']['expected_same_mean'], results['null_results']['expected_mixed_mean']]):.4f}")
    print(f"Δ(Same-Mixed): {results['test_results']['observed_difference']:.4f}")
    print(f"p-value: {results['test_results']['permutation_pvalue']:.4f}")
    print(f"Significance: {get_significance_stars(results['test_results']['permutation_pvalue'])}")
