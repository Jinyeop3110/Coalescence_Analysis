#!/usr/bin/env python3
"""
Generate vector decomposition map with varied coalescence trajectories
Each simulation uses a different random interaction matrix at u=0.6
...
def run_coalescence_with_trajectory(u_param, num_C, num_S, threshold=1e-3, seed=None):
    """
    Run a single coalescence simulation with a new random interaction matrix

    Returns:
        trajectory: list of (t, u, v, k) coordinates at different time points
    ...
    # Generate new interaction matrix for this simulation
    f_interaction = lambda: uniform_distribution(u_param, 0)
    I = np.zeros((N, N))
    for i in range(N):
        for j in range(N):
            I[i, j] = f_interaction()
    ...
    # Time points - focus on early dynamics
    t_final = 5000
    t_span = (0, t_final)
    # More time points in early phase
    t_eval = np.concatenate([
        np.linspace(0, 100, 20),
        np.linspace(100, 500, 15)[1:],
        np.linspace(500, 2000, 15)[1:],
        np.linspace(2000, t_final, 10)[1:]
    ])
...
def main():
    """Generate vector decomposition map with varied trajectories"""
    print("="*80)
    print("VECTOR DECOMPOSITION MAP WITH VARIED COALESCENCE TRAJECTORIES")
    print("="*80)
    ...
    print(f"  Note: Each simulation uses a different random interaction matrix")

    # Run simulations and collect trajectories
    print(f"\nRunning {num_reps} coalescence simulations with varied interaction matrices...")
    all_trajectories = []
    all_final_u = []
    all_final_v = []
    ...
    # Create the plot
    print(f"\nCreating vector decomposition plot with varied trajectories...")
    ...
    contourf = ax.contourf(U, V, density_uv, levels=levels, cmap='Greys', alpha=0.4)
    ...
    for traj_idx, trajectory in enumerate(all_trajectories):
        ...
        # Plot trajectory
        ax.plot(u_traj, v_traj, linewidth=0.5, alpha=0.3, color='steelblue')

        # Mark ending point with small dot
        ax.plot(u_traj[-1], v_traj[-1], 'o', markersize=2,
               color='darkblue', alpha=0.6, markeredgewidth=0)

    # Mark the theoretical starting point
    sqrt2_inv = 1.0 / np.sqrt(2)
    ax.plot(sqrt2_inv, sqrt2_inv, '*', markersize=15,
           color='gold', markeredgecolor='black', markeredgewidth=1,
           label=f'Initial coalescence state', zorder=10)

    # Add diagonal line
    ax.plot([0, 1], [0, 1], 'k--', linewidth=1.5, alpha=0.6, label='u = v (equal contribution)')
    ...
    ax.set_title(f'Coalescence Trajectories: Varied Interaction Matrices\n(u = {u}, n = {len(all_trajectories)} realizations)',
    ...
    # Save figure
    output_file = f'{dynamics_dir}/VectorDecomp_u{u}_varied_trajectories.svg'
    ...
    print(f"\nCreating detailed trajectory plot (subsample of 30)...")

    fig2, ax2 = plt.subplots(1, figsize=(100*mm, 100*mm), facecolor='w')

    # Plot only 30 trajectories for clarity
    n_show = min(30, len(all_trajectories))
    cmap = plt.cm.plasma
    ...
    for traj_idx in range(n_show):
        ...
        # Mark start and end
        ax2.plot(u_traj[0], v_traj[0], 'o', markersize=5,
                color=color, alpha=0.8, markeredgewidth=0.5, markeredgecolor='black')
        ax2.plot(u_traj[-1], v_traj[-1], 's', markersize=5,
                color=color, alpha=0.8, markeredgewidth=0.5, markeredgecolor='black')

    # Mark theoretical starting point
    ax2.plot(sqrt2_inv, sqrt2_inv, '*', markersize=18,
            color='gold', markeredgecolor='black', markeredgewidth=1.5,
            label=f'Initial state', zorder=10)

    # Add diagonal
    ax2.plot([0, 1], [0, 1], 'k--', linewidth=1.5, alpha=0.6, label='u = v')
    ...
    ax2.set_title(f'Individual Coalescence Trajectories\n(u = {u}, showing {n_show} of {len(all_trajectories)} realizations)',
                 fontsize=10, fontweight='bold')
    ...
    # Save second figure
    output_file2 = f'{dynamics_dir}/VectorDecomp_u{u}_varied_trajectories_detailed.svg'
    ...
    dist_from_start = np.sqrt((all_final_u - sqrt2_inv)**2 + (all_final_v - sqrt2_inv)**2)
    print(f"\nDistance from initial state (1/√2, 1/√2):")
    print(f"  Mean: {np.mean(dist_from_start):.3f} ± {np.std(dist_from_start):.3f}")
    print(f"  Range: [{np.min(dist_from_start):.3f}, {np.max(dist_from_start):.3f}]")

    print(f"\n📁 Output directory: {dynamics_dir}")
    print(f"📊 Generated files:")
    print(f"   - VectorDecomp_u{u}_varied_trajectories.svg")
    print(f"   - VectorDecomp_u{u}_varied_trajectories_detailed.svg")
    print("="*80)
