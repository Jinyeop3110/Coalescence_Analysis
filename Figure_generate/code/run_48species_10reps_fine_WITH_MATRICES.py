#!/usr/bin/env python
"""
48 Species Simulation with 10 Repetitions and Fine Intensity Intervals
FAST VERSION: Saves full interaction matrices with only 10 reps

This script runs Lotka-Volterra simulations with:
- 48 total species
- 12 species per community
- 10 repetitions per interaction strength (FAST)
...
def simulate_48_species_10reps_fine_with_matrices():
    """Run the 48 species simulation with 10 repetitions and fine intensity intervals.

    FAST VERSION: Only 10 reps per intensity for quick completion.
    """

    # Simulation parameters for 48 species - 10 repetitions, fine intervals
    session_name = "Simulation_Data/48species_10reps_fine_WITH_MATRICES"
    ...
    # Parameters
    N_reps = 10  # Number of repetitions per interaction strength (FAST VERSION)
    ...
    print(f"\n🔬 FAST 10-REP SIMULATION WITH INTERACTION MATRICES")
    print(f"=" * 70)
    ...
    print(f"\n⚡ FAST VERSION: Only 10 reps per intensity")
    print(f"   Expected completion time: ~20-30 minutes")
    print(f"   Expected file size: ~20-30 MB")
    print(f"=" * 70)

    # Check if we need to run the simulation
    output_file = session_name + '/Community_10reps_fine_WITH_MATRICES.json'
    ...
    params_dict = {
        ...
        't': t,
        'threshold': threshold,
        'simulation_type': 'fast_10reps_with_matrices',
    ...
    for i, u in enumerate(u_list):
        print(f"\n{'='*70}")
        print(f"Processing u = {u:.2f} ({i+1}/{len(u_list)})")
        print(f"{'='*70}")
        ...
        for rep in range(N_reps):
            ...
            print(f"  Rep {rep+1}/{N_reps} | Progress: {task_count}/{total_tasks} ({100*task_count/total_tasks:.1f}%) | ETA: {eta_minutes:.1f} min")
            ...
            # Store results for this repetition with full interaction matrix
            all_results[f"{u:.2f}"][f"rep_{rep:03d}"] = {
                ...
                "parameters": {
                    ...
                    "interaction_matrix": I.tolist()  # Full 48×48 matrix
                }
            }

            rep_end_time = time.time()
            print(f"    Completed in {rep_end_time - rep_start_time:.1f} sec")
    ...
    print(f"  • Repetitions per intensity: {N_reps}")
    print(f"  • Total simulations: {len(loaded_results) * N_reps}")
    ...
    print(f"  • Interaction matrix: {len(first_rep['parameters']['interaction_matrix'])}×{len(first_rep['parameters']['interaction_matrix'][0])}")
    ...
    print(f"    Mean: {first_rep['parameters']['interaction_matrix_stats']['mean']:.4f}")
    print(f"    Std: {first_rep['parameters']['interaction_matrix_stats']['std']:.4f}")

    print(f"\n🎯 Ready for analysis!")
    print(f"   All {len(loaded_results) * N_reps} interaction matrices are saved.")


if __name__ == "__main__":
    simulate_48_species_10reps_fine_with_matrices()
