#!/usr/bin/env python
"""
Large Species Pool Simulation for Natural Communities (500 Species)
...
python run_simulation_500_species_for_natural.py
"""
...
try:
    ...
    # Simple replacement for tqdm if not available
    def tqdm(iterable, total=None):
        for i, item in enumerate(iterable):
            if total and i % max(1, total//20) == 0:  # Print progress every 5%
...
def main():
    ...
    if Intention_to_reset or not os.path.isfile(session_name + '/Community.json'):
        ...
        with mp.Pool(processes=4) as pool:  # Use fewer processes due to memory constraints
