#!/usr/bin/env python
"""
Plot Phase Diagrams for Simulations with Interaction Matrices

This script creates phase diagrams from JSON simulation data that includes
full interaction matrices.

Supports:
- 10 reps data: run_48species_10reps_fine_WITH_MATRICES.py
- 500 reps data: run_48species_500reps_fine_WITH_MATRICES.py (future)

Outputs:
- Figure/PhaseDiagram/Fig_phase_diagram_10reps_WITH_MATRICES.svg
- Figure/PhaseDiagram/Fig_phase_diagram_500reps_WITH_MATRICES.svg
"""
...
def plot_phase_diagrams_with_matrices():
    """Generate phase diagrams for simulations with interaction matrices."""

    print("\n" + "="*70)
    print("GENERATING PHASE DIAGRAMS FOR SIMULATIONS WITH MATRICES")
    print("="*70)
    ...
    json_sessions = [
        {
            "name": "10reps_WITH_MATRICES",
            "json_file": "Simulation_Data/48species_10reps_fine_WITH_MATRICES/Community_10reps_fine_WITH_MATRICES.json",
            "description": "10 reps × 24 intensities (0.05-1.2, step 0.05) with full matrices",
            "output_suffix": "10reps_WITH_MATRICES"
        },
        {
            "name": "500reps_WITH_MATRICES",
            "json_file": "Simulation_Data/48species_500reps_fine_WITH_MATRICES/Community_500reps_fine_WITH_MATRICES.json",
            "description": "500 reps × 24 intensities (0.05-1.2, step 0.05) with full matrices",
            "output_suffix": "500reps_WITH_MATRICES"
        }
    ]
    ...
    for session in json_sessions:
        ...
        if not Path(session['json_file']).exists():
            ...
            print(f"   Skipping this session (may not be generated yet)")
            failed_plots += 1
            continue
        ...
        # Set up plotting parameters (same for both 10 and 500 reps)
...
def main():
    ...
    # Check 10 reps data
    json_10 = "Simulation_Data/48species_10reps_fine_WITH_MATRICES/Community_10reps_fine_WITH_MATRICES.json"
    if Path(json_10).exists():
        try:
            with open(json_10, 'r') as f:
                data_10 = json.load(f)
            total_10 = sum(len(data_10[u_key]) for u_key in data_10.keys())
            print(f"  ✓ 10 reps data: {total_10} simulations available")
        except:
            print(f"  ⚠️  10 reps data: File exists but cannot read")
    else:
        print(f"  ✗ 10 reps data: Not found")

    # Check 500 reps data
    json_500 = "Simulation_Data/48species_500reps_fine_WITH_MATRICES/Community_500reps_fine_WITH_MATRICES.json"
    if Path(json_500).exists():
        try:
            ...
            print(f"  ✓ 500 reps data: {total_500} simulations available")
        except:
            print(f"  ⚠️  500 reps data: File exists but cannot read (may be running)")
    else:
        print(f"  ✗ 500 reps data: Not found (run simulation first)")

    print()

    # Generate phase diagrams with available data
    plot_phase_diagrams_with_matrices()
