#!/usr/bin/env python
"""
Ablation Study: Carrying Capacity Heterogeneity with std=0.2
...
- Carrying capacity: N(1, 0.2^2), clipped to positive values
  - Mean = 1
  - Std = 0.2
  - CV = 0.2
...
python run_ablation_carrying_capacity_std02.py
"""
...
def carrying_capacity_gaussian_std02():
    """
    Generate Gaussian random carrying capacity with mean=1, std=0.2.
    Clips to minimum of 0.01 to avoid division by zero.
    """
    k = np.random.normal(1.0, 0.2)
    return max(0.01, k)
...
def simulate_ablation_carrying_capacity():
    """Run the ablation study with carrying capacity heterogeneity (std=0.2)."""

    session_name = "Simulation_Data/48species_ablation_k_std02"
    ...
    print(f"\n" + "="*70)
    print("ABLATION STUDY: CARRYING CAPACITY HETEROGENEITY (std=0.2)")
    print("="*70)
    print(f"Carrying capacity distribution: N(1, 0.2^2), clipped to >0.01")
    ...
    output_file = session_name + '/Community_ablation_k_std02.json'
    ...
    params_dict = {
        ...
        't': t,
        'threshold': threshold,
        'carrying_capacity_distribution': 'N(1, 0.2^2)',
        'carrying_capacity_mean': 1.0,
        'carrying_capacity_std': 0.2,
    ...
    for i, mu in enumerate(mu_list):
        ...
        for rep in range(N_reps):
            ...
            seed = int(mu * 10000) + rep * 100000 + 5000000  # Different seed offset
            np.random.seed(seed)
            ...
            I, g, k = InitializeSpeceiesPool(N,
                                           ...
                                           f_k=carrying_capacity_gaussian_std02,  # Heterogeneous K
    ...
    print(f"\n{'='*70}")
    print("CARRYING CAPACITY ABLATION (std=0.2) SIMULATION COMPLETED!")
    print(f"{'='*70}")
