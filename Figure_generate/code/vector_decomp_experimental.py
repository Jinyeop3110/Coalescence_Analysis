#!/usr/bin/env python3
"""
Vector decomposition analysis for experimental coalescence events.
Converted from new_Plot_vectorDecomp.ipynb
"""
...
output_dir = "/Users/jysong/Desktop/Gore_lab/Sequencing/Coalescence_session_20230404/Figure_generate/code/Figure/VectorDecomp_exp"
...
def main():
    ...
    print("Starting vector decomposition analysis for experimental data...")
    ...
    for c_i, type_name in enumerate(['LN_12', 'MN_12', 'HN_12']):
        ...
        if to_plot_null and len(null_counts) > 0:
            ...
            fig.savefig(f"{output_dir}/ClassFractions_{type_name}_exp_GroupedBarPlot.svg", bbox_inches='tight')
            ...
            if to_plot_subperturb:
                ...
                fig.savefig(f"{output_dir}/ClassFractions_{type_name}_exp_subperturb_GroupedBarPlot.svg", bbox_inches='tight')
            
            f=PolarizedPlot(data1,data2, c_i, colors)
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_style_Polarized.svg',bbox_inches='tight')

            f=PolarizedThetaPlot(data1,data2, c_i, colors)
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_style_PolarizedTheta.svg',bbox_inches='tight')

            f=thetaplot(data1, data2, colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_style_Theta.svg',bbox_inches='tight')
            
            f=rplot(data1, data2, color=colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_style_R.svg',bbox_inches='tight')

            f= PolarizedPlot(data_null_model_1,data_null_model_2, c_i, colors)
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_null_style1_Polarized.svg',bbox_inches='tight')

            f = thetaplot(data_null_model_1, data_null_model_2, colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_null_style1_Theta.svg', bbox_inches='tight')
            
            f = rplot(data_null_model_1, data_null_model_2, color=colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_null_style1_R.svg', bbox_inches='tight')
            
            f= PolarizedPlot(data_subperturb_1,data_subperturb_2, c_i, colors)
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_subperturb_style1_Polarized.svg',bbox_inches='tight')

            f = thetaplot(data_subperturb_1,data_subperturb_2, colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_subperturb_style1_Theta.svg', bbox_inches='tight')
            
            f = rplot(data_subperturb_1, data_subperturb_2, color=colors[c_i])
            f.savefig(f'{output_dir}/Metric_metric3_{type_name}_exp_subperturb_style1_R.svg', bbox_inches='tight')
