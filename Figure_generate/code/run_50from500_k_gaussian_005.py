#!/usr/bin/env python
"""
Run 500-species simulation with k_gaussian_0.05 parameters.
50 species per community from 500 total species, non-overlapping communities,
Gaussian interaction distribution with σ=0.05 (very tight distribution).
"""
...
def run_50from500_k_gaussian_005():
    """Run 50from500 with k_gaussian_0.05 parameters."""
    
    print("Running 50from500 simulation with k_gaussian_0.05...")
    
    # Parameters
    N = 500  # Total species
    num_C = 10  # Number of communities 
    num_S = 50  # Species per community (non-overlapping)
    sigma = 0.05  # Very tight Gaussian std for interactions
    ...
    # Output directory
    session_name = "Simulation_Data/new_k_gaussian_0.05_defined_pool_nooverlap_50from500_natural_full"
    os.makedirs(session_name, exist_ok=True)
    
    # Generate interaction matrix with k_gaussian_0.05 parameters
    print(f"Generating {N}×{N} interaction matrix with σ={sigma} (very tight)...")
    np.random.seed(123)  # Different seed from 0.15 version
    ...
    print(f"Base interaction: {base_interaction} ± {sigma} (very tight distribution)")
    ...
    for u_idx, u in enumerate(tqdm(u_values, desc="Interaction strengths")):
        ...
        for iteration in range(n_iterations):
            np.random.seed(iteration + u_idx * 1000 + 5000)  # Different seed offset
                ...
                if sum1 > threshold and sum2 > threshold and sum_mixed > threshold:
                    try:
                        ...
                        if len(coeffs) == 2:
                            ...
                            type_results['c'].append(0.005)  # Smaller residual for tight distribution
                            
                    except np.linalg.LinAlgError:
                        type_results['a'].append(0.5)
                        type_results['b'].append(0.5)
                        type_results['c'].append(0.005)
                else:
    ...
    print(f"\n✅ 50from500 k_gaussian_0.05 simulation complete!")
    ...
    print(f"🎯 Non-overlapping communities with σ={sigma} (very tight Gaussian interactions)")
    
    return session_name

if __name__ == "__main__":
    print("🧪 Starting 50from500 k_gaussian_0.05 simulation...")
    print("=" * 70)
    ...
    try:
        session_name = run_50from500_k_gaussian_005()
        end_time = time.time()
