...
READ BEFORE RUNNING (updated 2026-07-31)
----------------------------------------
Two things changed after this script was written, and running it unmodified
would undo both:

1. Extended Data figures were renumbered so main-text citation order is
   monotonic: old 8 -> 4, 4 -> 5, 5 -> 6, 6 -> 7, 7 -> 8. The output names
   below have been updated to the new numbering. Do not restore the old names.

2. This script rasterises every panel to a 300 dpi JPEG before assembling
   (see pdf_to_image). Extended Data Fig. 5 is no longer built that way: it is
   composed as vector art by rebuild_and_fit_a4.py, which also keeps it inside
   A4. The Fig. 5 block below is therefore disabled.

Every display item must fit easily within A4 (210 x 297 mm) per the journal's
requirements. This script does not enforce that. After regenerating anything
here, run rebuild_and_fit_a4.py, which is idempotent and will scale any
oversized output back into A4.
"""
...
def main():
    ...
    # ED Fig 4: Assembly effect simulation (was ED Fig 7 before renumbering).
    # Single file, no panel labels needed.
    copy_file('ED_Fig5_assembly_effect_simulation.pdf', 'ED_Fig4_combined.pdf')

    # ED Fig 5: parental-community richness (was ED Fig 4 before renumbering).
    # Deliberately not built here. This block rasterised the six source panels
    # to a 300 dpi JPEG, which put visible ringing on panel f's small per-bar
    # annotations, and it produced a 360.5 x 348.1 mm page that does not fit A4.
    # rebuild_and_fit_a4.py now composes it as vector at 200.0 x 223.7 mm.
    #
    # It also drew panel f from ED_Fig4f_species_ablation_bar.pdf, whose
    # archived copy is the January version labelled "CLS" / "Mixing" /
    # "Species per community". Those violate writing_rules.md. The corrected
    # panel lives in panel_sources/ and is the one rebuild_and_fit_a4.py uses.

    # ED Fig 6: Pairwise selection correlation simulation (was ED Fig 5).
    combine_figure(
        input_files=[
    ...
    # ED Fig 7: Pairwise selection correlation experimental (was ED Fig 6).
    combine_figure(
        input_files=[
        ...
        grid_layout=[[0, 1, 2]],
        output_name='ED_Fig7_combined.pdf',
        labels=['a', 'b', 'c']
    )

    # ED Fig 8: pH vs outcome (was ED Fig 7's asset; single file, a/b panels
    # already present). Number unchanged, source file name unchanged.
