...
- **Source**: regenerated in `Figure_generate/Draft/v5/latex/supplementary_figs/Fig_R1_1B_OD_vs_PDI.pdf`
- **Code**: `Figure_generate/Draft/v5/public_code_repo/figures/supplementary/make_supp_fig14_od_ph.py`
- **Description**: Two-row Supplementary Fig. 14. Top row: signed parental-community OD difference versus PDI, separately for Nutr-, Base, and Nutr+. Bottom row: direct endpoint OD$_{600}$ versus endpoint pH across parental-community replicates in each medium. The bottom row was added in the second revision round to address the suggested OD-pH check without treating reused parent pairs as independent observations.
