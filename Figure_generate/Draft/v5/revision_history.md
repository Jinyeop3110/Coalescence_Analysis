...
## 2026-08-09 — Accepted Supplementary Figs. 14--20 audit revisions; Supplementary Fig. 13 retained

- **Affected point**: author figure/caption guidance for Supplementary Figs. 13--20.
- **Scope decision**: Supplementary Fig. 13 was intentionally left unchanged, including its use of “pool size,” as requested by the author.
- **What changed**:
  - Supplementary Fig. 14 now has lowercase panel letters, a signed $\Delta\mathrm{OD}_{A-B}$ axis, and a caption that distinguishes independent events from parent-label-swapped reflections.
  - Supplementary Fig. 15 now uses event-clustered bootstrap intervals for grouped summaries and one paired A-minus-B contrast per event for continuous inference, eliminating duplicated parent-level pseudoreplication.
  - Supplementary Fig. 16 now has lowercase panel letters, descriptive column titles, medium names rather than internal abbreviations, and clarified independent-event statistics.
  - Supplementary Fig. 17 now reads only the intended 54-well A1--F9 isolate layout, interprets the workbook entry `48?` as 4.8, excludes off-layout controls, and removes unsupported isolate-to-ASV assignments.
  - Supplementary Fig. 18 now analyzes only the synthetic-community abundance table, uses taxonomy from the processed synthetic-sequence workbook, and no longer infers monoculture acidifier/alkalinizer identities without a verified well-to-ASV mapping.
  - Supplementary Figs. 19--20 now use lowercase panel letters, sentence-case labels, sample standard deviations, and captions with exact sample sizes, Gini coefficients and ASV richness summaries.
- **Synchronization**: regenerated assets, captions, supplementary methods and provenance were synchronized between `latex/` and `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/`.
- **Type**: figure-generation code, figure assets, captions, supplementary methods, provenance and packaging inputs.

---

## 2026-08-09 — Accepted Supplementary Figs. 3--10 audit revisions and Extended Data Fig. 7 titles

- **Affected point**: author figure/caption guidance for Supplementary Figs. 3--10 and Extended Data Fig. 7.
- **What changed**:
  - Supplementary Fig. 3 now treats each full-matrix simulation as the independent unit for the within-versus-between comparison (`n = 10` paired simulations), removes duplicated diagonal blocks, reports the paired Wilcoxon result, and separates lowercase panel letters from titles.
  - Supplementary Figs. 4--6 now carry lowercase panel letters and informative in-artwork titles (`σg`, `σK`, Gaussian/Gamma); captions describe outcome-fraction sweeps rather than phase diagrams.
  - Supplementary Figs. 7--9 now use neutral isolate indices I1--I12, apply the Methods thresholds without a fallback assignment, display an Unassigned category, and distinguish 132 attempted directional assays from 122 returned directional assays (61 unordered pairs). The same accounting is stated in the main Fig. 4 caption.
  - Supplementary Figs. 10--12 now use lowercase panel letters, manuscript metric notation, and captions that distinguish independent events from the reflection-symmetrized PDI histogram values.
  - Extended Data Fig. 7 now has `Nutr-`, `Base`, and `Nutr+` titles above panels a--c. Titles were overlaid on the curated vector panel sources; the panels were not regenerated with the known-unfiltered plotting script, so established `n = 90/83/83` data remain intact.
- **Synchronization**: regenerated supplementary artwork and captions were copied to both `latex/` and `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/`; the Extended Data composite was distributed to all four active destinations.
- **Type**: figure-generation code, vector/raster assets, captions, main figure legend, provenance and packaging inputs.

---

## 2026-08-09 — Supplementary Figs. 27--29 relabelled; Extended Data Figs. 5--6 given informative panel titles

- **Affected point**: author figure-labelling guidance. No numbered reviewer point.
- **Files changed**:
  - `Figure_generate/code/generate_pie_plots.py`
  - `Figure_generate/code/common_setup.py`
  - `Figure_generate/code/plot_phase_diagram_ablation_species_number.py`
  - `Figure_generate/code/make_ed_fig5_combined.py`
  - the nine `coalescence_matrix_{medium}_s{6,12,24}.pdf` assets in both Supplementary Information trees
  - `latex/figures/extended_data/ED_Fig{5,6}_combined.pdf`, distributed to all four active Extended Data destinations
  - `latex/supplementary_sections/{figures,extended_data}.tex` and the clean submission mirrors
  - `latex/supplementary_figs/file_source.md` and its clean submission mirror
- **What changed**:
  - Removed internal acquisition IDs such as `P8-53` from Supplementary Figs. 27--29 and indexed parental communities as C1, C2, and so forth within each replicate. Captions now define the indexing convention.
  - Added `$\mu = 0.3$`, `$\mu = 0.6$`, and `$\mu = 0.8$` titles to Extended Data Fig. 6a--c.
  - Added `Initial richness = 4`, 6, 12, 24, and 48 titles to Extended Data Fig. 5a--e. These panels each sweep $\mu$ from 0.05 to 1.20, so fixed `$\mu = XX$` titles would be scientifically incorrect. Corrected the caption's obsolete description of a--e as fixed-$\mu$ similarity maps; they show outcome fractions across $\mu$ at different initial richness values.
- **Verification**: visually inspected a regenerated coalescence matrix and both Extended Data composites; all nine matrices pass the figure-format audit at exactly 6.00 pt and RGB; new Extended Data titles print within the 5--7 pt range; ED 5 and ED 6 line weights pass at 0.28--0.57 pt and 0.73--0.98 pt, respectively.
- **Type**: figure-generation code, supplementary and Extended Data figure assets, captions and provenance.

---

## 2026-08-09 — Supplementary Fig. 1 regenerated and caption corrected to match its source data

- **Affected point**: author guidance on Supplementary Fig. 1. No numbered reviewer point.
- **Files changed**:
  - `Figure_generate/code/build_phylogeny.py`
  - `latex/supplementary_figs/Fig_1-3_phylogeny_tree.{pdf,svg}`
  - `latex/supplementary_figs/file_source.md`
  - `latex/supplementary_sections/figures.tex`
  - `latex/sections/{results,methods}.tex`
  - `latex/supplementary_sections/supplementary_methods.tex`
  - the corresponding figure, provenance and caption files in `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/`
  - the corresponding Results and Methods files in `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/`
- **What changed**:
  - Added horizontal clearance between terminal labels and the abundance column. ASV1 has the longest branch, and its `ASV1 (Lactococcus)` label previously overlapped the ASV1 abundance rectangle, making its placement appear incorrect.
  - Changed the on-figure title from 50 bacterial isolates to 50 unique ASVs.
  - Replaced the obsolete maximum-likelihood/full-length/EMBL-EBI caption with the method and counts implemented in the generator and recorded in `Species_phylogeny.xlsx`: a neighbor-joining identity-distance tree based on 252--253-bp 16S rRNA amplicon sequences, representing 50 unique ASVs among 54 isolates, four phyla and 21 families.
  - Aligned main Results, main Methods and Supplementary Methods to the same source-backed description. Direct workbook recount confirms four phyla: 28 Proteobacteria, 10 Firmicutes, 10 Bacteroidota and 2 Actinobacteriota (ASV55, \textit{Streptomyces}; ASV56, \textit{Curtobacterium}).
  - Updated the generator's automatic synchronization destinations from obsolete `Draft/v3` to both active `v5` trees.
- **Verification**: regenerated with the Biopython-enabled `coalescence` environment; visually inspected the standalone figure and compiled submission page; confirmed identical figure hashes in the generator output and both `v5` trees; figure-format audit passes at 5.71--6.66 pt and RGB; both Supplementary Information sources compile successfully at 63 pages.
- **Type**: figure-generation code, figure asset, supplementary caption and provenance.

---

## 2026-08-09 — Simulation mu = 0.6 three-bar outcome comparison generated

- **Affected point**: second-round Reviewer 2 clarification of Dominance versus evidence from prior assembly history.
- **Files changed**:
  - `latex/revision_second_round/revision_figure_folder/make_fig1_outcome_comparison.py`
  - `latex/revision_second_round/revision_figure_folder/make_fig1_outcome_comparison_simulation_mu0p6.py`
  - `latex/revision_second_round/revision_figure_folder/fig1_outcome_comparison_simulation_mu0p6.{pdf,svg,png}`
  - `latex/revision_second_round/revision_figure_folder/fig1_outcome_comparison_simulation_mu0p6_counts.csv`
  - `latex/revision_second_round/revision_figure_folder/source.md`
- **What changed**: added a simulation counterpart to the experimental three-bar mock-up at interaction-strength parameter value $\mu = 0.6$. The generator reads the assembly-history JSON outputs directly rather than hard-coding displayed counts. Coalescence gives 295/96/108 Dominance/Mixture/Restructuring events ($n=499$), direct assembly gives 99/93/308 ($n=500$), and an event-matched simple additive null constructed from the same valid coalescence parental-community vectors gives 1/498/0 ($n=499$).
- **Provenance choice**: the null bar is calculated from the same simulation run as the coalescence bar. The separate 68/1,200 simple-additive-null result at $\mu=0.6$ was not mixed into this panel because it comes from a different 200-replicate simulation dataset.
- **Layout follow-up**: moved the three bar labels from above the panel to below the bars on the simulation version, while retaining the top labels in the experimental reference panel.
- **Type**: response/main-figure candidate artwork and provenance infrastructure. No manuscript or clean submission source currently references this draft panel.

---

## 2026-08-08 — Extended Data Fig. 7 Nutr+ sample size reverted to n = 83; code-audit item 3b resolved as incorrect

- **Affected point**: internal code audit item 3b. No numbered reviewer point.
- **Files changed**:
  - `latex/supplementary_sections/extended_data.tex`
  - `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/supplementary_sections/extended_data.tex`
- **What changed**:
  - Reverted the Nutr$+$ panel (c) sample size from `\revsecond{$n = 90$}` back to
    a plain `$n = 83$`. The `\revsecond` wrapper was dropped because this restores
    the originally published value rather than making a second-round change.
  - Added one `\revsecond` sentence stating that the metric additionally requires
    at least three species above the detection threshold in the coalesced
    community, so Nutr$+$ uses 83 of the 90 events classified in Fig. 4d.
- **Why**: the earlier change to `n = 90` was made on the strength of code-audit
  item 3b and was a regression. Re-running the per-event analysis with the
  quality-control exclusion list applied reproduces the published caption exactly
  on all three panels: Nutr$-$ n = 90, $\Delta$ = 0.0164, p = 0.307; Base n = 83,
  $\Delta$ = 0.2354; Nutr$+$ n = 83, $\Delta$ = 0.1407. The caption's
  $\Delta = 0.141$ is the value obtained from the 83-event set, so `n = 90`
  left the caption internally inconsistent.
- **Event accounting** (94 wells per medium): Nutr$-$ loses 4 to the $\geq$3-species
  validity filter, which are the same 4 the QC list drops, giving 90. Base loses 6
  with no sequence data and 5 to QC, giving 83. Nutr$+$ loses 7 to the validity
  filter and 4 to QC (P6-02, P6-47, P6-57, P6-74), giving 83. Base and Nutr$+$
  arriving at the same 83 is a coincidence of different filters, not a copied value.
- **Not changed**: the ED Fig. 7 artwork carries only significance stars and no
  sample sizes, so no figure was regenerated.
- **Known bug, not fixed here**: `Figure_generate/code/plot_correlation_barplots_clean.py`
  imports `Coalescence_data` from `common_setup` but never applies `exception_list`.
  Re-running it today yields 90/88/87 with $\Delta$ = 0.016/0.230/0.136 and silently
  includes nine QC-failed wells; this produced the mismatched
  `correlation_summary_clean.csv` noted in `CAMPAIGN.md` 10.4.
- **Type**: supplementary caption edit (both trees); audit correction

---

## 2026-08-08 — Tier 1 word-reduction draft revised after context and style review

- **Affected point**: editor request to shorten the Introduction, Results and
  Discussion toward 3,500 words. No numbered reviewer point.
- **Files changed**:
  - `latex/revision_second_round/length_reduction_review_v2.tex` and compiled PDF
  - `latex/revision_second_round/length_reduction_review_tier1b.tex` and compiled PDF
  - `latex/revision_second_round/word_reduction_plan.md`
- **What changed**:
  - Reviewed all ten Tier 1 proposals against their surrounding paragraphs,
    revision-marked text, response-letter quotations and citation placement.
  - Replaced the unsafe full deletion of the Introduction results preview with
    a two-sentence synthesis that retains the interaction-strength transition,
    correlated persistence, gLV support, nutrient dependence in synthetic and
    natural sample-derived communities, and both mechanistic regimes.
  - Preserved the species-level/community-level contrast in I-2 and the
    assembly-filtering-to-coupled-fates bridge in I-4; kept `Debray2022` attached
    to the claim it supports.
  - Softened the categorical priority-effects comparison, preserved the
    biochemical-versus-phenomenological model caveat, and polished the Methods
    pointers and paragraph transitions.
  - Corrected the plan's obsolete claim that live legends are inline in
    `results.tex`; both manuscript trees now use `sections/figure_legends.tex`.
  - Revised Tier 1 savings from 389 to 365 words. With Tier 1B, the plan reaches
    3,860 words from its recorded 4,359-word baseline, leaving about 360 words
    for Tier 2.
- **Type**: editorial planning and author-review material. The active manuscript
  and clean submission manuscript were deliberately not edited in this pass.
- **Verification**: both author-review TeX documents compiled successfully with
  `latexmk -pdf -interaction=nonstopmode -halt-on-error`; rendered headings and
  totals were checked with `pdftotext` and representative pages were inspected.

---

## 2026-08-01 (fourth pass) — Supplementary Figure statistics-and-reproducibility checklist (ED-26); exact p-values deferred (ED-27)

- **Affected points**: editor's Supplementary Figure checklist (dot plots over bar charts, precise n, defined axes/errors/symbols/colour scales, named tests with exact p, repeat count for representative data). Tracker rows ED-26, ED-27. No reviewer point.
- **Files changed**: `latex/supplementary_sections/figures.tex` and its markup-stripped mirror `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/supplementary_sections/figures.tex`.
- **Type**: caption and front-matter text only. No figure asset was regenerated.

### What changed

Audited all 46 Supplementary Figure captions against the artwork itself, reading the on-figure text out of each PDF rather than trusting the captions.

**One new paragraph** at the head of the Supplementary Figures section fixes the conventions in a single place instead of across 46 captions: mean $\pm$ s.d.; box-plot centre line / box / whiskers; biological rather than technical replicates; two-sided tests; the ED Fig. 1 species colour map; no micrographs, gels or blots anywhere in the SI, so no scale bars or molecular-weight markers; and the reason overlaid dot plots do not apply to stacked outcome-fraction bars (each bar is one categorical proportion over independent events, not a mean over replicates, and the counts are already printed as n/total).

**Twenty caption edits.** The most consequential: Supplementary Figs. 4, 5 and 6 had **no colour legend at all** — the stacked Dominance/Mixture/Restructuring fractions were unlabelled in both the artwork and the caption — and now carry the key plus n = 1,200 events per μ. Supplementary Fig. 32's caption said "Points show individual simulation runs", but the generating script subsamples 100 points per group for legibility; the caption now says so. Sample sizes were added to Figs. 7-9 (61 assayed isolate pairs of 66 possible), 36, 37, 38, 39, 40, 41 and 42; representative-example denominators to Figs. 2, 30 and 31; the test name and the bar-to-point relationship to Fig. 15; and a descriptive-only note to Fig. 23.

Every simulation count was read from source rather than inferred: `run_ablation_*.py` (`N_reps = 200`, `num_C = 4`, giving six pairs per pool), the R3_3 result JSONs (`N_POOLS = 200`), `sim_parent_norm_events.csv` (24 μ values, 1,200 events each) and `Community_10reps_fine_WITH_MATRICES.json` (24 μ, 10 pools each). Phase-diagram colours came from `COLORMAP.get_phase_diagram_colors()` and the fill order in `common_setup.create_phase_diagram`.

The submission-tree copy was regenerated by brace-matched stripping of the live file and diffed against the previous copy: the diff contains exactly these 20 changes and nothing else, confirming the two trees were in sync beforehand. Both documents compile clean at 63 pages.

### What was deliberately not done

The sentence "exact p values are reported either on the figure or in the legend" was **withheld** from the new paragraph, because it is not yet true. Supplementary Fig. 18 prints `p = 0.000` in ten panels and names no test; Fig. 44 shows only `***`; Fig. 13a,b name a Kruskal-Wallis test but print no p; Fig. 22 prints `p<0.001` six times with no test named. These need the generating scripts re-run, along with n per OD tertile (Fig. 15), the isolate count (Fig. 17) and n for Figs. 27-29, none of which are recoverable from the tree. Tracked as ED-27.

---

## 2026-08-01 (third pass) — Extended Data line weights brought to spec (ED-15); `parent`/`parents` permitted in figure labels (ED-14); SI panel-label audit (ED-16)

- **Affected points**: editorial artwork checklist, tracker rows ED-14, ED-15, ED-16. No reviewer point.

### ED-15 — line weights

- **Requirement**: "Lines and strokes should be set between 0.25 and 1 pt" (Extended Data guide). This is a separate measure from the text-size/colour campaign and had never been verified, so a new checker was written: `figure_format_campaign/measure_linewidths.py`. Because Extended Data publishes standalone at native size, the width in the PDF content stream is the printed width and no scale model is needed.
- **Baseline 4 / 8 compliant.** ED 6 reached 2.95 pt, ED 8 2.00, ED 7 1.77, ED 3 1.50.
- **Fix**: every offending stroke capped at 0.9 pt in the four panel scripts, through a named constant carrying the spec quotation. 0.9 rather than 1.0 so run-to-run canvas drift from `bbox_inches='tight'` cannot push a figure back over the ceiling.
- **Two of the offending widths were never written down in any script** — the marker traces in ED 6 panel d and the reference `axhline` in ED 8 carried no explicit `linewidth` and inherited matplotlib's 1.5 default. Found by matching measured composite widths against placement scale rather than by reading the scripts, which would have missed them.
- **Result: 8 / 8 within 0.25-1 pt.**
- **Files changed**
  - `Figure_generate/code/Figure_revision/R3_1_additive_null/analyze_additive_null.py` (ED 3)
  - `Figure_generate/code/make_ed_fig5_combined.py` (ED 6 panels)
  - `Figure_generate/code/plot_correlation_barplots_clean.py` (ED 7 panels)
  - `Figure_generate/code/plot_ph_figures_revised.py` (ED 8)
  - `latex/figures/extended_data/ED_Fig{3,6,7,8}_combined.pdf`, redistributed by `rebuild_and_fit_a4.py` to all four Extended Data directories including the submission tree.
  - New: `figure_format_campaign/measure_linewidths.py`, `figure_format_campaign/changelog/WS-LINEWEIGHT.md`.

### ED-14 — resolved against the writing rule, not the figure

- **Author decision (2026-08-01)**: the rule was wrong, not the artwork. `parent`/`parents` may be used as standalone nouns **inside figure labels**, where the full term "parental community" wraps and drives the label under the 5 pt floor. Prose and captions are unchanged and still require the full term.
- Labels standardised to sentence case: **"Same parent"** / **"Cross parents"** in ED 6 (panels a-d) and ED 7 (panels a-c). ED 6 panel d also moved its legend entries and its two axis titles to sentence case; it was the only part of that figure in Title Case, against panels a-c.
- **The real defect was in the caption.** The ED 6 caption quoted its own legend as "Same parental community" / "Cross-community", wording that has never appeared in the artwork. A reader following the caption to the figure would not have found the text it names. Now quotes the labels as drawn.
- **Files changed**
  - `latex/writing_rules.md` rule 4, and `revision.rule.md` rule 12: exception recorded with its rationale, plus a new requirement that a caption quoting a figure label reproduce it as drawn.
  - `latex/supplementary_sections/extended_data.tex` **and** the submission tree's copy: ED 6 caption.
  - ED 6 / ED 7 panel scripts as above.

### ED-16 — the 8 pt lowercase panel-label rule does apply to Supplementary Information

- **Author decision (2026-08-01)**: yes, the 8 pt bold lowercase panel-label rule and the 5-7 pt text rule apply to the SI, not only to main display items and Extended Data. ED-16 therefore stops being an open question and becomes actionable.
- **Body text is already essentially done**: `verify_figures.py` reports 57 / 65 at 5-7 pt print, and every genuine failure is an already-documented blocked item in `CAMPAIGN.md` 10.3. ED 6 and ED 7 appear in the fail list only because the verifier flags their 8 pt panel letters, which are correct per spec; the verifier predates that finding and does not model the exception.
- **Panel labels are the open work.** New audit `figure_format_campaign/audit_panel_labels.py` measures case, weight, uprightness and print size for every panel letter. Result: of the 16 figures with detectable labels, **3 comply and 13 do not** — 29 uppercase letters, 2 not bold, 35 at the wrong print size (5.89-6.89 pt against a required 8 pt). 49 figures have no detectable letters, being single-panel or drawing them as paths.
- The audit needed two false-positive filters, both found by inspecting its own first pass: oblique single letters are mathtext variables, not labels; and candidates must form a distinct contiguous `a, b, c` run in one font and size, which is what separates the real labels of `natural_taxonomic_distinctness` from the repeated `C`/`D` category marks beside them. A geometric "nothing above and to the left" test was tried first and discarded because it discards every label below the first row.
- **Not yet executed**, and one structural obstacle is recorded for decision in the tracker: in `interaction_matrix_post_assembly` the panel letter is fused with descriptive text in a single call (`'A\nBefore\nassembly'`, and `'C  Pooled coefficients'`), so the letter cannot be taken to 8 pt without splitting the label in two, which is a layout change rather than a size change.
- **Type**
  - Figure assets (Extended Data, all four directories)
  - Supplementary caption text, both trees
  - Project writing rules
  - Workflow infrastructure (two new verifiers)

---

## 2026-08-01 (third pass) — Funding statement corrected; Competing Interests confirmed compliant as-is

- **Affected point**: editor's competing-interests instruction, and an author query on whether the funding statement had ever been checked. Tracker row ED-22.

### Competing Interests: no change needed

The editor asked for a detailed statement, identical in the manuscript and the tracking system. An expanded negative declaration was drafted (naming initials, patents, consultancies, stock, unpaid advocacy, and funder non-involvement) but the author elected to keep the existing single sentence, "The authors declare no competing interests." That already satisfies the policy requirement that a negative statement be included where none exist, and it is present and identical in both trees and in the delivered PDF. Nothing was edited. The remaining obligation is portal-side: the same sentence must be entered verbatim in the tracking system.

### Funding: never verified before today, and wrong in three ways

The sentence had been carried unexamined since at least `Archive/v4_may_14`. The only prior action on it was structural: relocation from Acknowledgements into a separate `\section*{Funding}` on 2026-07-31 at the editor's request. No tracker row had ever covered its correctness.

Three defects, all corrected:

- **Funder names were informal.** "the Schmidt Polymath Award" named no real entity; the program is *Schmidt Science Polymaths*, run by Schmidt Sciences (verified against schmidtsciences.org). "the Sloan Foundation" is properly the *Alfred P. Sloan Foundation*. Exact strings matter because funder names are matched against the Crossref Funder Registry for open-access and compliance tracking.
- **Two of three authors were undeclared.** Only J.G. was covered, while Author Contributions credits J.Y.S. with performing the experiments and all the data analysis. The author confirmed J.Y.S. and J.H. are members of J.G.'s group rather than holders of personal awards, so the correct fix is a group attribution, not per-author "was supported by" lines. Those lines are reserved for personal funding such as an external fellowship, and inventing them would imply awards that do not exist.
- **No award or grant numbers**, which Nature requests where applicable. Not inserted, since none exist anywhere in the tree and none may be fabricated.

Final wording, in both trees: "J.G. acknowledges funding support from a Schmidt Science Polymath Award from Schmidt Sciences and from the Alfred P. Sloan Foundation. These awards also supported J.Y.S. and J.H."

A `\todo{}` was added in the live tree, mirrored as a TeX comment in the submission source, carrying the three unresolved questions: insert award numbers if the funders require them; confirm no other Gore lab grant paid salaries or consumables for this project, since any such grant must be listed; add a personal-fellowship sentence only if one exists.

- **Files changed**
  - `latex/sections/methods.tex`; `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/sections/methods.tex`
  - Refreshed `00_submit_new/{Main_Manuscript.pdf, Main_difference.pdf, Main_Manuscript_compressed.pdf, Main_Manuscript_Revised_LaTeX_Source.zip}`
- **Type**
  - Manuscript text
  - Submission package
- **Verification**
  - Both documents rebuilt with `latexmk -pdf -halt-on-error`: exit 0 each.
  - Delivered `Main_Manuscript.pdf` carries the full corrected sentence; the obsolete string "Schmidt Polymath Award" is absent.
  - "TODO" appears 0 times in the delivered submission PDF and once in the live `main.pdf`, which is the intended asymmetry.
  - Compressed PDF rebuilt with the established `/ebook` settings; zip regenerated to the same 25-file manifest.
  - Competing Interests confirmed byte-identical in both trees and rendered in the delivered PDF.

### Open item carried forward

The funding statement lists two funders and no grant numbers. If the Gore lab holds other grants that supported this project, they are currently undeclared. This is the one part of the statement that cannot be checked from the repository and needs an author answer.

---

## 2026-08-01 (second pass) — Data Availability rewritten against the *minimum dataset* guidance; deposit DOIs added to the Reference list

- **Affected point**: editor's data-availability instruction, second and more specific version. Tracker row ED-17.
- **Trigger**: the guidance requiring that the statement "make the conditions of access to the minimum dataset that are necessary to interpret, verify and extend the research transparent to readers", that supplying data through Supplementary Information is "strongly discouraged" in favour of repositories, that the statement "reference any source data published alongside the paper", and that "if DOIs are provided, we also require including these in the Reference list (authors, title, publisher (repository name), identifier, year)".

### What the earlier pass today had left non-compliant

The morning pass satisfied the policy's deposition and full-URL requirements but not this one. Three gaps remained: the statement was a bare pointer that never said what the deposit contains, so the minimum dataset was not transparent; it led with "available in the Supplementary Information", which is the delivery route the guidance strongly discourages; and neither DOI appeared in the Reference list.

### Changes

- **Data Availability now enumerates the deposit** file class by file class: raw demultiplexed 16S V4 fastq reads, the DADA2 ASV count table, ASV relative-abundance tables for synthetic and natural communities, per-event and per-community tables with OD/pH/diversity, experimental metadata and coalescence recipes, and the simulated event tables. Enumeration is deliberately restricted to what is deposited rather than asserting per-figure coverage, on the author's instruction.
- **Supplementary Information removed as a data route.** Data are now sourced from Dryad only.
- **Source data addressed explicitly**: "Source data for the figures are provided within this deposit; no separate Source Data files accompany this paper."
- **Raw reads placed in Dryad, on the author's instruction**, resolving the contradiction where the manuscript said reads were on Dryad while `public_code_repo/data/README.md` said they were deposited separately under a manuscript accession that never existed. That README section was rewritten to match, and now documents the `COALESCENCE_FASTQ` re-run route.
- **Both DOIs added to `references.bib`** as `SongDryad2026` and `SongZenodo2026`, cited from the two statements. `naturemag.bst` has no `format.doi` function and silently drops `doi = {}` fields (verified: no DOI appears anywhere in `main.bbl`), so the identifier is carried in `note` instead. Both render as authors, title, repository name, identifier, year.

### Regression caught and reverted within this pass

The rewrite reintroduced "Bacterial isolates and assembled communities are available from the corresponding author upon reasonable request", which the morning pass had removed on the author's instruction as the statement's only request-only qualification. Reverted in both trees before the final build. The delivered PDFs were checked for the string directly, not just the source.

- **Files changed**
  - `latex/sections/methods.tex`; `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/sections/methods.tex`
  - `latex/references.bib`, synced to both submission sources
  - `public_code_repo/data/README.md`
  - Refreshed `00_submit_new/{Main_Manuscript.pdf, Main_difference.pdf, Main_Manuscript_compressed.pdf, Main_Manuscript_Revised_LaTeX_Source.zip}`
- **Type**
  - Manuscript text
  - Submission package
  - Public code repository documentation
- **Verification**
  - Both `main.tex` documents rebuilt with `latexmk -pdf -halt-on-error`: exit 0, 0 undefined citations, 0 undefined references.
  - `SongDryad2026` and `SongZenodo2026` confirmed present in both `main.bbl` files and rendering with all five required elements.
  - All three delivered PDFs grepped: "Dryad Digital Repository" and "no separate Source Data files" present, "upon request" absent.
  - `Main_Manuscript_compressed.pdf` rebuilt with the established Ghostscript settings (`/ebook`, `-dCompatibilityLevel=1.5`): 2.31 MB against 2.30 MB before.
  - Zip regenerated to the same 25-file manifest, 16.7 MB, unchanged from the previous archive.
  - Supplementary document untouched: its `references.bib` was synced for consistency but no supplementary text cites either deposit, so `Supplementary_Information.pdf` and its zip were deliberately not rebuilt.
- **Verified claim, and one that could not be verified**
  - Confirmed true: the public repository does contain the read-processing code. `pipeline/01_sequencing/Merging-SeqWorkflow_ForCoalescence.R` runs the full DADA2 workflow from demultiplexed fastq to ASV tables, assigning taxonomy against SILVA 138, matching the Methods.
  - Could not be confirmed from the tree: that the fastq files are actually present in the Dryad deposit. Written on the author's instruction. See the open item below.

### Open items carried forward

- **The fastq files must actually be in the Dryad deposit.** The statement now asserts it and `data/README.md` now documents it; nothing in the working tree can verify the deposit's contents.
- **Fig. 5c source data are still not deposited.** `public_code_repo/pipeline/04_figure_generation/fig5/README.md` records that the pairwise invasion assay outcomes are not in the Dryad archive, so that panel cannot be regenerated from the deposit. The enumeration avoids claiming per-figure coverage, but "Source data for the figures are provided within this deposit" is only strictly true once those outcomes are uploaded. Same underlying blocker as R1-code.
- The SRA/INSDC exposure recorded in the morning entry below is unchanged and still accepted by the author.

---

## 2026-08-01 — Data and Code Availability statements revised against the Nature data-availability policy; main-text drift back-ported

- **Affected point**: editor's data/code availability instruction. New tracker row ED-17.
- **Trigger**: the editor's note asking that data and code be deposited in a public repository, that any request-only access be justified in the statement and in correspondence with the editor, and pointing at https://www.nature.com/nature-research/editorial-policies/reporting-standards#availability-of-data

### Drift back-ported first (prerequisite)

Comparing the two trees markup-blind showed all 14 supplementary sections in sync but **five main-text files carrying 25 copy-edits that existed only in the submission tree** and had never been returned to `latex/sections/`: `results.tex` (11), `figure_legends.tex` (7), `methods.tex` (5, counting the Dryad sentence), `introduction.tex` (2), `discussion.tex` (2). Article/tense/agreement polish in every case, with the submission wording the better one ("this structured interaction" to "this interaction structure"; "two-sided Fisher exact test" to "two-sided Fisher's exact test"; "as reference" to "as the reference database"). Editing live-first and re-stripping to submission would have silently reverted all 25, so they were back-ported before any availability edit was made. Each replacement was applied only where it matched exactly once, and the wording changes sit inside the existing `\rev{}`/`\revsecond{}` spans rather than disturbing them.

After the back-port the only surviving difference between the trees is deliberate: live keeps `\todo{Acknowledgements text needed...}` visible, the submission copy holds the same note as a LaTeX comment. Acknowledgements text is still outstanding.

### Statement changes (applied to both trees)

- **DOIs now display as full URLs** in both statements. The policy requires that "dataset identifiers including DOIs should be expressed as full URLs"; both `\href{}`s previously rendered a bare DOI as their link text. The Zenodo identifier is the concept DOI, confirmed against `public_code_repo/CITATION.cff`, which is the correct one to cite since it always resolves to the latest release.
- **Named the code license.** The policy asks for an OSI-approved license; the repository carries MIT (`public_code_repo/LICENSE`, `CITATION.cff`) but the statement had never said so. Code Availability now ends "All code is released under the MIT License."
- **Data Availability rewritten and expanded by the author** over the course of this pass, superseding the shorter draft made earlier in the same session. It now opens on the Dryad deposit, then itemises what the deposit contains (raw demultiplexed 16S V4 fastq reads; the DADA2 ASV count table; ASV relative-abundance tables for synthetic and natural communities; per-event and per-community optical density, pH and diversity tables; experimental metadata and coalescence recipes; simulated coalescence event tables), states explicitly that figure source data live inside the deposit and that no separate Source Data files accompany the paper, and points to Code Availability for the DADA2 workflow and panel-generation code.
- **Isolates and communities**: the original "available upon request" sentence was removed on the author's instruction, then the author reinstated a softened form, "Bacterial isolates and assembled communities are available from the corresponding author upon reasonable request", as the closing sentence. Live cultures cannot be deposited, so a request route is unavoidable here; recorded so the wording is not silently dropped again.
- **Two `@misc` entries added to `references.bib` in both trees**, `SongDryad2026` and `SongZenodo2026`, so the deposits are cited in the reference list as the policy asks for code ("cited in the reference list"). Both statements now carry `\citep{}` alongside the hyperlink. The two bib files are byte-identical across trees (md5 `4894d9c2...`).

On the live side the Data Availability block moved from `\rev{}` to `\revsecond{}`, since the whole statement was rewritten this round. Code Availability kept its existing split, with the additions inside the established `\revsecond{}` span. The `\citep{}` calls sit outside the colour macros in both statements, so the citation marks render in the normal body colour while the surrounding new prose stays blue.

### Open compliance risk, accepted by the author

Nature's reporting-standards page lists data types whose deposition in a named community repository is mandatory, and **DNA/RNA sequencing data is on that list, with NCBI SRA or the Trace Archive as the named repositories**. Raw 16S reads currently sit on Dryad only; a project-wide grep found no SRA, ENA, DDBJ or BioProject accession anywhere in `v5`. The April planning documents, now under `Draft/Archive/`, did carry the correct sentence ("deposited in the NCBI Sequence Read Archive under BioProject accession PRJNA[XXXXXX]"), but the placeholder was never filled and the sentence was dropped by `v5` in favour of Dryad.

Raised with the author on 2026-08-01, who elected to stay Dryad-only and not add a justification paragraph. Recorded here because the editor may return it: generalist repositories are accepted for the minimum dataset in general, but not as a substitute for the mandated repository when the data type is listed. No placeholder accession was written into either tree, so nothing can ship half-filled.

- **Files changed**
  - `latex/sections/{discussion,figure_legends,introduction,methods,results}.tex` (back-port); `latex/sections/methods.tex` (statements); `latex/references.bib`
  - `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/sections/methods.tex`, `.../references.bib`
  - Refreshed `00_submit_new/{Main_Manuscript.pdf, Main_difference.pdf, Main_Manuscript_compressed.pdf, Main_Manuscript_Revised_LaTeX_Source.zip}`
- **Type**
  - Manuscript text
  - Submission package
- **Verification**
  - Markup-blind comparison of all 20 section files across both trees: 19 identical, `methods.tex` differing only by the intended `\todo` versus comment. `references.bib` md5-identical.
  - Both `main.tex` documents rebuilt with `latexmk -pdf -halt-on-error`: 29 pages each, 0 undefined citations, 0 undefined references. Page count unchanged despite two added references.
  - Both PDFs render the new statements identically, and both bibliographies contain the two new deposit entries.
  - `Main_Manuscript.pdf` and `Main_difference.pdf` confirmed md5-identical to the clean and marked-up `main.pdf` respectively, so the copy direction is right.
  - `Main_Manuscript_compressed.pdf` rebuilt with the established Ghostscript settings (`/ebook`, `-dCompatibilityLevel=1.5`), 2.31 MB against 2.30 MB before, 29 pages, new statements present in the extracted text.
  - Zip regenerated to the same 25-file manifest as the previous archive, verified name by name. It shrank from 30.2 MB to 16.7 MB because the old archive still carried a stale 15.8 MB `main.pdf` against the current 8.8 MB build; the file list is unchanged.
  - **Ordering hazard worth noting**: the compressed PDF and the zip are derived from `Main_Manuscript.pdf` and the source tree, so a source rebuild silently staled them mid-pass. Caught by comparing mtimes rather than assuming, and both were regenerated last. Refresh in the order sources to `main.pdf` to the two copies to compressed to zip.
  - Supplementary document untouched, so `Supplementary_Information.pdf`, its zip, and `Extended_Data_Figures.zip` were deliberately not rebuilt.

---

## 2026-07-31 — Extended Data figures renumbered to restore citation order (editorial checklist)

- **Affected point**: editorial display-item requirements. New tracker row ED-5.
- **Trigger**: the editor requires each Extended Data item to be cited in order in the main text. It was not. First-citation order was 1, 2, 3, **8**, 4, 5, 6, 7.
- **Root cause**: the out-of-order citation sits inside the `\revsecond{}` block added this round for R2-1, which cites Extended Data Fig. 8 at `results.tex` L46, ahead of the first citations of 4 through 7. This was a regression introduced by the current revision round, not a pre-existing defect.
- **Second, independent defect found while fixing this.** Extended Data 7 and 8 had their asset filenames crossed against their caption labels: the block captioned "Extended Data Fig. 7" pulled `ED_Fig8_combined.pdf`, and the block captioned "Extended Data Fig. 8" pulled `ED_Fig7_combined.pdf`. The compiled SI was correct because the two errors cancelled, but the standalone files delivered in `Extended_Data_Figures/` were named by the wrong numbers. Since the journal publishes Extended Data as separate files, ED 7 and ED 8 would have appeared against each other's captions. Confirmed by rendering both assets before touching them.
- **Renumbering applied** (old label to new): 1, 2, 3 unchanged; 8 to 4; 4 to 5; 5 to 6; 6 to 7; 7 to 8. Chosen because it is exactly the order of first citation, so it fixes the ordering violation and the filename crossing in a single pass. After the change `ED_FigN_combined.pdf` holds the figure captioned "Extended Data Fig. N" for every N, which was not true before.
- **Files changed**
  - `latex/supplementary_sections/extended_data.tex` and its clean-source counterpart: eight figure blocks physically reordered, caption labels, `\label{}`s and `\includegraphics` paths renumbered.
  - Citations renumbered in `latex/sections/results.tex`, five files under `latex/supplementary_sections/`, and the matching files in both clean submission sources.
  - Assets renamed in all four Extended Data directories: `latex/figures/extended_data/`, both submission `figures/extended_data/`, and `revision_submission/00_submit_new/Extended_Data_Figures/`.
- **Type**
  - Manuscript text
  - Supplementary text
  - Figure assets
  - Submission package
- **Verification**
  - Main-text first-citation order is now 1 2 3 4 5 6 7 8, monotonic.
  - All eight blocks appear in numerical order, each paired with the matching `ED_FigN_combined.pdf`.
  - Asset content re-checked by page size after renaming, since the rename script's first pass was wrong: it applied the label permutation to filenames, which was invalid precisely because 7 and 8 were already crossed. Caught by verifying content rather than trusting the script, and corrected by swapping `ED_Fig4` and `ED_Fig8` in all four directories.
  - Rendered SI page 24 to confirm visually that Extended Data Fig. 4 now carries the assembly-history figure with its own caption.
  - All four documents recompiled: 0 undefined citations, 0 undefined references, 0 errors, 0 missing files.
- **Page count: the main manuscript is now 29 pages, was 30**
  - Cause: the six main figures were moved out of the Results body into a new `sections/figure_legends.tex`, input at the end of `main.tex` after Methods, in both the live and clean submission sources. Moving six full-width floats from inline positions to end matter repaginates the article, which accounts for the lost page.
  - Neither the title change nor the Extended Data renumbering affected pagination. Three clean-room builds confirm it: current source 29, old title 29, pre-renumber `results.tex` 29. That is the expected result, since every renumbered citation is a single digit and Computer Modern digits are tabular.
  - Recorded because an earlier draft of this entry misattributed the change to non-converged `latexmk` runs. All three clean-room controls post-date the figure-legend restructuring, so they could isolate the title and the renumbering but not the restructuring itself. The restructuring is the actual cause.
---

## 2026-08-01 — Extended Data resized to the Extended Data spec; panel letters corrected to 8 pt lowercase

- **Two specifications located that the editor's artwork PDF does not contain.** The linked "Guide to preparing final artwork" is silent on panel lettering and gives figure widths but not Extended Data heights. The governing text lives elsewhere:
  - [Preparing figures, our specifications](https://research-figure-guide.nature.com/figures/preparing-figures-our-specifications/): "Separate panels in multi-panelled figures should be labelled with 8-pt bold, upright (not italic) and lowercase a, b, c, etc." and "Maximum text size: 7 pt. Minimum text size: 5 pt."
  - [Extended Data guide](https://www.nature.com/documents/Extended_Data_guide.pdf): "Maximum page dimensions are 180 mm wide by 170 mm tall"; "Lines and strokes should be set between 0.25 and 1 pt"; "Files must be saved in RGB"; and, critically, Extended Data is "not edited or styled by our art department; for this reason, authors are requested to follow Nature's style guide as closely as possible."
- **Consequence worth recording**: main figures are redrawn to house style by an art editor where necessary, Extended Data is not. Whatever is supplied for Extended Data is what publishes, so these figures carry the compliance burden themselves.

### Panel letters restored to 8 pt

`LABEL_PT` had been lowered to 6.8 on the reading that the 7 pt ceiling covered panel letters. It does not: the specification names them as an exception at 8 pt. Restored to 8.0, with the quotation and URL recorded at the constant so it is not lowered again. ED 5, 6 and 7 rebuilt; their letters are now lowercase bold at 8.00 pt.

### Height cap corrected from the wrong table

Heights had been capped per figure at 185 to 210 mm from the main display-item caption-length table. Extended Data has its own flat limit of 170 mm, so that was the wrong table and had produced two over-tall figures. `HEIGHT_CAP_MM` is now a flat 170.

### ED 5 fitted without degrading panel f

A blanket scale to 170 mm would have been x0.814 on the whole page, dragging panel f's per-bar annotations from 4.3 pt to about 3.5 pt. Instead `build()` now shrinks only the square "cells" rows when a composite is too tall, leaving any full-width row at its size. Panel spacing was also tightened from 6/4/7/5.5 mm to 4/3/5/4, which the guide encourages: "Try to keep white space to a minimum when arranging panels within a figure."

Result: ED 5 is exactly 180.0 x 170.0 mm, panels a to e sit at 5.5 pt rather than 6.9, and panel f is unchanged at 4.4 pt against 4.3 before. The height rule is met and panel f is no worse.

### Outcome

| ED | size mm | letters | other text | status |
|---|---|---|---|---|
| 1 | 146.2 x 170.0 | in-panel | 6.8 | compliant; rescaling also brought its 8.4 pt table text inside 5 to 7 |
| 2 | 179.3 x 112.5 | in-panel | 6.9 to 7.0 | compliant |
| 3 | 180.0 x 74.0 | A, B, C at 6.4 to 6.9 | 6.4 to 6.9 | uppercase and undersized, both baked into the source panel |
| 4 | 180.0 x 139.3 | in-panel | 6.9 to 7.0 | compliant |
| 5 | 180.0 x 170.0 | a to f at 8.0 | 4.4 to 6.8 | panel f text under 5 pt, unresolvable without the plotting script |
| 6 | 180.0 x 146.2 | a to d at 8.0 | 6.7 | compliant |
| 7 | 180.0 x 67.5 | a to c at 8.0 | 6.7 | compliant |
| 8 | 139.2 x 64.4 | in-panel | 6.6 to 6.9 | compliant |

Six of eight fully compliant.
...
- **Verification**
  - All eight items within 180 x 170 mm; all four Extended Data directories byte-identical.
  - Both supplementary documents recompiled: 0 errors, 0 missing files. Now 61 pages, down from 62, because ED 1 and ED 5 shrank and content reflowed.
  - Extended Data captions still appear in order 1 to 8 and paired with the right artwork; ED 5's page rendered and inspected.
- **Newly surfaced, not yet addressed**
  - Line weights must sit between 0.25 and 1 pt. Four items breach it, all inherited from their source panels: ED 6 up to 2.88 pt, ED 8 to 2.00, ED 7 to 1.70, ED 3 to 1.50. Logged as ED-15.
  - ED 3's uppercase panel letters are inside its source panel, so they need the same treatment as ED-14.
- **Supplementary figures audited but deliberately unchanged**
  - Of 71: 22 carry uppercase panel labels, 30 have labels off 8 pt, 55 have text above 7 pt, 2 below 5 pt.
  - No change made, because the specifications quoted above are for main display items and Extended Data. Extended Data is described as part of the article, "included online within the full-text HTML and at the end of the online PDF"; Supplementary Information is a separate file and is generally published as supplied. No citation was found either way, so a 55-file cleanup was not started on an assumption. Logged as ED-16 pending confirmation from the editor.

---

## 2026-08-01 — Flattened PNG copies of the main figures removed from `latex/figures/`

- **Why**: each main figure had a sibling `figure_main_*.png` alongside its PDF, at 288 to 300 dpi and fully flattened. The artwork guide is categorical that bitmap formats, PNG named explicitly, cannot be used for vector art. Submitting one in place of its PDF would be worse than the current state: the whole figure becomes raster and no text stays editable. Removed so they cannot be picked up by mistake.
- **Checked before deleting**
  - No `.tex` or `.md` source references them.
  - Every `\includegraphics` names `.pdf` explicitly, so no extension-less include could resolve to a PNG.
  - All six are byte-identical to the copies already in `figure_format_campaign/backup_figs_20260801/figures/`, so nothing is lost. That directory belongs to the author's own campaign and was left untouched.
- **Files removed**: `latex/figures/figure_main_{1..6}_*.png`.
- **Verification**
  - Forced a full rebuild of `main.tex` with `latexmk -g`: 29 pages, 0 errors, 0 missing files.
  - Confirmed from `main.fls` that all six figures resolve to `.pdf`. Note that `\graphicspath` yields a doubled separator, `figures//figure_main_1_...pdf`, which a naive grep for `figures/figure_main` misses; matching on `figure_main` alone shows all six.
  - No `.png` remains in any figure directory across the live tree, both submission sources, or `Extended_Data_Figures/`.
  - `Main_difference.pdf` refreshed from the rebuilt `main.pdf`.
- **Note**: this is a hygiene change only. It does not address ED-8, ED-9 or ED-11, whose low-resolution and CMYK elements are embedded inside the figure PDFs themselves, not in these sibling files.

---

## 2026-07-31 — Artwork audited against the Nature guide; ED 6/7 vectorised, ED widths set to 180 mm, panel refs lowercased

- **Source**: the Nature branded research journals "Guide to preparing final artwork", linked by the editor. Fetched and read rather than recalled. Governing numbers: two-column width 180 mm (one-column 88 mm); figure text 5 pt minimum, 7 pt maximum; original research supplied in RGB; line art saved as vector, bitmaps placed at 300 dpi or better; 50 MB per figure; maximum height set by caption length, ~185 mm at <300 words, ~210 mm at <150, ~225 mm at <50.
- **Affected tracker rows**: ED-10, ED-12, ED-13 closed. ED-8, ED-9, ED-11 remain with the author.

### Correction: the first raster audit overstated the problem

The initial pass computed effective dpi by dividing each image's pixel dimensions by its bounding box. That is wrong for any image placed under a rotation, because the bounding box axes then correspond to the opposite pixel axes. It also counted inline images, which in these files are 1x1 px solid fills and 5 px wide colour-ramp strips where dpi carries no meaning.

Recomputed from each image's placement matrix, restricted to rasters that actually carry detail (at least 1000 px) and deduplicated by xref:

| Figure | detail rasters | below 300 dpi | min dpi | colour |
|---|---|---|---|---|
| 1 | 16 | 9 | 88 | CMYK, gray |
| 2 | 8 | 0 | 300 | gray |
| 3 | 1 | 0 | 423 | gray |
| 4 | 1 | 0 | 300 | gray |
| 5 | 0 | n/a | n/a | vector only |
| 6 | 6 | 0 | 300 | CMYK |

So the resolution problem is **Fig. 1 alone**, not Figs. 1, 2, 4 and 6 as first reported. Fig. 6's photographs are exactly 300 dpi; the earlier 192 dpi reading was the rotation error. Fig. 2's "17 dpi" and Fig. 4's "162 dpi" were inline gradient strips and small inline images, not degraded artwork.

The CMYK finding is unaffected and still applies to Figs. 1 and 6.

Fig. 1's nine sub-threshold rasters are rasterised stacked bar charts, confirmed by extracting and viewing them. Since they are line art, the guide wants them vector; raising their dpi would satisfy the number but not the rule.

The text-size measurements were not affected by this error. They were validated against a control: ED 5's panel labels, which this project sets at exactly 8.0 pt, are reported as 8.0 pt.

### ED-10, Extended Data 6 and 7 rebuilt as vector

- Both were single flat JPEGs, 2488x1758 and 2551x817, with all text outlined and nothing editable. They are bar and scatter charts, so the guide requires vector.
- Rebuilt from the archived vector source panels through `rebuild_and_fit_a4.py`, which now composes ED 5, 6 and 7. ED 6 from the four correlation-versus-$\mu$ panels, ED 7 from the three per-medium panels.
- **All eight Extended Data items are now vector: 0 embedded rasters across the set.** ED 6 fell from 221 KB to 26 KB, ED 7 from 128 KB to 79 KB, and the supplementary PDF from 3,365 KB to 3,124 KB.

### ED-12, widths retargeted from 200 mm to 180 mm

- The earlier 200 mm target came from fitting A4 with a margin, before the artwork guide was consulted. 180 mm is the guide's two-column standard and also sits inside A4, so it satisfies the editor's requirement and the artwork requirement together.
- Height is now capped per figure from its caption word count rather than by one global number. Caption lengths were measured: ED 1 74 words, ED 2 138, ED 3 228, ED 4 97, ED 5 135, ED 6 229, ED 7 192, ED 8 159. Recheck the caps if a caption crosses 50, 150 or 300 words.
- Result: every item at 180.0 mm wide except ED 8 at 138.9 mm, which is already narrower. Tallest is ED 1 at 209.3 mm against its 210 mm cap, then ED 5 at 203.9 mm against 210 mm.

### ED-13, panel references lowercased in the body text

- The paper was inconsistent three ways: artwork uppercase, every caption lowercase, body text 28 uppercase references and 0 lowercase. Nature style is lowercase, and the captions were already correct.
- Lowercased the panel letter following each `\figref{}`, 26 in `results.tex` and 2 in `methods.tex`, mirrored in the clean submission source: 56 substitutions in total. The regex lowercases only a letter run terminated by a non-letter, so a capital beginning a following word cannot be caught; verified beforehand that no such case exists.
- Body text and captions now agree. The **artwork still carries uppercase panel letters in all six main figures**, so that layer remains inconsistent until those files are regenerated.
...
- **Type**
  ...
  - Submission package
- **Verification**
  - Re-audited every figure with PyMuPDF, measuring effective dpi against each image's placement box rather than the page, which an earlier pass got wrong and which understated resolution by more than an order of magnitude.
  - All four documents recompiled: 0 undefined citations, 0 undefined references, 0 errors, 0 missing files. Main 29 pages, supplementary 62.
  - Panel references confirmed lowercase in the rendered PDF, not just the source.
  - All four Extended Data directories byte-identical.
- **Deliberate exception**: panel labels on the rebuilt figures are 8 pt, above the guide's 7 pt ceiling. The 5 to 7 pt range governs text inside the figure; panel letters are conventionally set slightly larger, and 8 pt matches Nature house style.
- **Found, not fixed**: ED 6 and ED 7 label their axes "Same Parent" and "Cross Parents". Standalone `parent`/`parents` are barred by `writing_rules.md` rule 4 and by `revision.rule.md` rule 12, and the ED 6 caption already quotes the legend as "Same parental community" and "Cross-community", so caption and artwork disagree. This is pre-existing, not introduced by the rebuild. Fixing it means editing text inside the source panels, which risks breaking their layout, so it is left for a decision. Logged as ED-14.

---

## 2026-07-31 — Extended Data Fig. 5 rebuilt as vector; all display items fitted to A4 (editorial checklist)

- **Affected point**: editorial display-item requirements, tracker row ED-6.
- **Requirement**: every display and Extended Data item must fit easily within A4 (210 x 297 mm). Five of eight Extended Data items did not. All six main figures already did.
- **Script**: `latex/figures/extended_data/rebuild_and_fit_a4.py`, run under `~/miniforge3/bin/python`, which is the interpreter carrying PyMuPDF. Idempotent: re-running leaves already-fitted items untouched.
- **Extended Data Fig. 5 rebuilt rather than merely shrunk**
  - The previous asset came from `combine_extended_figures.py`, which rasterises every panel to a 300 dpi JPEG before assembling. Line art and the small per-bar annotations in panel f carried visible JPEG ringing. The rebuild places the six archived source panels with `show_pdf_page`, so the figure is now fully vector: 0 embedded rasters, 5 embedded fonts, 70 KB against 669 KB.
  - The old page was 360.5 x 348.1 mm because panel f is natively 302.8 mm wide and the old layout added wide margins around it. The new layout gives panel f the full content width and trims the margins, landing at 200.0 x 223.7 mm. Panel f now occupies 92 percent of the figure width rather than 84, so at A4 it renders about 5 percent larger than before.
  - Source panels copied to `latex/figures/extended_data/panel_sources/` so the rebuild is self-contained and does not reach into `Draft/Archive/`.
  - **Terminology regression caught during the rebuild.** The archived panel f, and the copy in `latex/supplementary_figs/`, are the January version whose legend reads "CLS" and "Mixing" and whose axis reads "Species per community". All three violate `writing_rules.md`. Building from it would have reverted terminology fixes made for this revision. Replaced with the corrected June version from `Figure_generate/code/Figure/StackedBar_ClassFractions/`, whose legend reads Dominance, Mixture, Restructuring and whose axis reads "Parental-community richness". Verified in the rebuilt asset by text extraction and by rendering.
  - A true font-size increase for panel f is not available: its SVG stores text as outlined paths with no font metadata, and no plotting script for it survives in the tree. The gains here are the removal of rasterisation, the larger panel-f share, and native A4 fit.
- **Remaining items scaled to fit**: target box 200 x 287 mm, leaving margin inside A4 so the fit is comfortable rather than exact. ED 1 x0.779, ED 2 x0.990, ED 4 x0.900, ED 6 x0.949, ED 7 x0.926. ED 3 and ED 8 already fitted and were left alone. Scaling draws the source page into a smaller page, so vector content stays vector; nothing was rasterised.
- **Files changed**: `ED_Fig1..8_combined.pdf` in all four Extended Data directories, kept byte-identical across them (verified by checksum of the whole set). New `rebuild_and_fit_a4.py` and `panel_sources/` under `latex/figures/extended_data/`.
- **Type**
  - Figure assets
  - Submission package
  - Workflow infrastructure
- **Verification**
  - All eight Extended Data items and all six main figures now fit A4. Largest remaining is ED 5 at 200.0 x 223.7 mm.
  - All four Extended Data directories hold an identical asset set.
  - All four documents recompiled: 0 undefined citations, 0 undefined references, 0 errors, 0 missing files. Main 29 pages, supplementary 62.
  - Supplementary PDF shrank from 3,963 KB to 3,365 KB, consistent with ED 5 losing its embedded JPEG.
  - Re-audited the four editorial requirements after the author's concurrent figure-legend restructuring: 6 main display items against a limit of 6; 8 Extended Data items against a limit of 10; Extended Data first-citation order 1 2 3 4 5 6 7 8; every item within A4.
- **Extended Data directories tidied**
  - `combine_extended_figures.py` was left encoding the pre-renumbering output names, so running it would have silently reverted the renumbering, re-rasterised the panels, and rebuilt Fig. 5 from the January panel f with its "CLS"/"Mixing" labels. Output names updated to the new numbering, the Fig. 5 block disabled in favour of `rebuild_and_fit_a4.py`, and a header added recording all three hazards.
  - Removed stray `main.aux`, `main.fdb_latexmk`, `texput.fls` and `texput.log` from `latex/figures/extended_data/` and from `Supplementary_Information_LaTeX_Source/figures/extended_data/`. These were stubs left by an accidental `pdflatex` run in those directories on 2026-07-29 and were unreferenced by any source file. The copies in the supplementary submission source mattered most, since that directory ships.
  - All four Extended Data directories now hold exactly the eight assets, byte-identical across directories. The live directory additionally holds the two build scripts and `panel_sources/`, which are inputs rather than deliverables.
- **Editorial display-item requirements now fully met**, audited end to end after the figure-legend restructuring: 6 of 6 main display items; 8 of 10 Extended Data items; Extended Data first-citation order 1 2 3 4 5 6 7 8; all 14 items within A4, largest ED 5 at 200.0 x 223.7 mm.
- **Noted, not a defect**: neither `main.tex` source references `figures/extended_data`; the assets are consumed only by the supplementary document. The copy under `Main_Manuscript_Revised_LaTeX_Source/figures/extended_data/` is therefore unused by that document and adds about 575 KB to `figures.zip`. Kept in sync deliberately, on the author's instruction.
- **Still open**
  - Seven assets in `latex/supplementary_figs/` still carry "CLS" or "Mixing" labels: `Fig_S12_species_ablation_stacked_bar`, `Fig_S25_assembly_effect_simulation_stacked_bar`, `Fig_S4_robustness_metrics_stacked_bar`, `Fig_assembly_effect_experimental`, `Fig_assembly_effect_simulation`, `Fig_natural_communities_coalescence`, `overview_ablation_species`. `Fig_S12` is not referenced by any supplementary section and appears to be an orphan; the others need checking against the sections that include them before the terminology can be called clean.

---

## 2026-07-31 — Title revised at the editor's request

- **Affected point**: editorial (title), not a numbered reviewer point. New tracker row ED-4.
- **Files changed**
  ...
  - `revision_submission/00_submit_new/Supplementary_Information_LaTeX_Source/supplementary.tex`
- **What changed**
  - The editor proposed a revised title and asked that the manuscript files be updated to match. Applied verbatim as supplied, including its sentence case, which differs from the Title Case of the previous version and matches Nature house style:
    - Previous: Interspecies Interactions Drive Community-Level Selection in Microbial Coalescence
    - Current: Interspecies interaction strength affects community-level selection in microbial coalescence
  - Marked `\revsecond{}` in the two live sources so the change is visible in the marked-up difference PDF; left unmarked in the two clean submission sources, which are markup-stripped and do not define the macro. Same split previously used for the Zenodo DOI in Code Availability.
  - Retained the previous title as a source comment beside each `\title{}` so the change stays auditable in the TeX itself.
  - Updated the response-letter cover page so the submission package does not carry two different titles.
  - **Refreshed all five delivered PDFs in `00_submit_new`.** Each is a copy of a compiled source, so all five still carried the old title after the source edits. `Main_difference.pdf` from `latex/main.pdf` (marked-up), `Main_Manuscript.pdf` and `Supplementary_Information.pdf` from the two clean submission sources, `Response_to_Reviewers.pdf` from the second-round response letter.
  - `Main_Manuscript_compressed.pdf` regenerated with Ghostscript 10.05.0. The setting was recovered by matching output size against the previous file: `-dPDFSETTINGS=/ebook -dCompatibilityLevel=1.5` reproduces it to within 300 bytes (3,781,751 versus 3,782,050), whereas `/printer` overshoots to 4.29 MB. Used `/ebook`.
- **Type**
  - Manuscript text
  - Response text
  - Submission package
- **Verification**
  - Compiled all four manuscript documents plus the response letter with `latexmk -pdf -f -interaction=nonstopmode`. All five exit 0; 0 undefined citations, 0 undefined references, 0 errors in every log.
  - Page counts unchanged: main 30, supplementary 62, response letter 6. The new title sets on two lines in both main and SI, as the previous one did.
  - Confirmed the rendered string via `pdftotext` on page 1 of all five PDFs rather than relying on exit status.
  - Re-verified the same string on all five delivered PDFs in `00_submit_new` after refreshing them; 5 of 5 carry the new title, page counts 30/30/30/62/6.
  - Ghostscript emits `warning: ignoring zlib error: incorrect data check` on this input. Benign and pre-existing, since the previous compressed file was produced by the same version from the same source. Confirmed the rebuild is intact: extracted text matches the uncompressed manuscript to 13 characters out of 66,921, and rendered pages are correct. The image count rises from 104 to 164 because `pdfwrite` re-tiles images, which is expected.
  - `Response_to_Reviewers.pdf` carries no WORKING DRAFT banner. The 2026-07-30 entry listed it as stale and still bannered; that was already out of date before this change, and it is now regenerated regardless.
- **Still open**
  - `public_code_repo/CITATION.cff` and `public_code_repo/README.md` still carry the old title, as does the Zenodo record minted from them (concept DOI 10.5281/zenodo.21696637). Updating these needs a commit to the separate public repository and, for the archive metadata to change, a new Zenodo release. Deliberately not done here.
  - `response_tailoring_policy.md` and the `revision_first_round/` files retain the old title as historical record. `00_submit_prev` likewise left untouched.
  - The stale `Main_Manuscript_Revised_LaTeX_Source.zip`, `Supplementary_Information_LaTeX_Source.zip`, and `figures.zip` were not regenerated, so they still contain the old title. These were already stale before this change; the main zip carries the marked-up variant rather than the clean source of the same name. Regenerate them as part of final packaging, once the editorial checklist arrives.

---

## 2026-07-31 — Author affiliations updated; manuscript reordered to the editor's required section list

- **Affected reviewer points**: none. Editorial/production request (author metadata verification, and the required main-manuscript section order).
- **Files changed**
  - `latex/sections/title_abstract.tex`; `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/sections/title_abstract.tex`
  - `latex/sections/results.tex`; same file in the clean submission source
  - `latex/sections/figure_legends.tex` (new); same file in the clean submission source
  - `latex/main.tex`; clean submission `main.tex`
  - `latex/sections/methods.tex`; clean submission `sections/methods.tex`
  - `revision_submission/00_submit_new/{Main_Manuscript.pdf, Main_difference.pdf, Main_Manuscript_compressed.pdf, Main_Manuscript_Revised_LaTeX_Source.zip}` (regenerated)
- **What changed**
  - **Affiliations.** All three authors now carry affiliation [1], "Physics of Living Systems, Department of Physics, Massachusetts Institute of Technology, Cambridge, MA, USA". Jiliang Hu additionally carries [2], "Department of Mechanical Engineering, Massachusetts Institute of Technology, Cambridge, MA, USA", so the byline reads Jinyeop Song$^1$, Jiliang Hu$^{1,2}$, Jeff Gore$^1$. No author was added, removed, or reordered, so the editor's approval-documentation warning does not apply.
  - Jinyeop Song's Department of Physics affiliation is carried by the combined [1] string rather than repeated as a separate entry, which would have printed "Department of Physics, MIT" twice.
  - **Section order.** Audited the main manuscript against the editor's required list. Title, author list, affiliations, abstract, main text, Methods, Data Availability, Code Availability, Acknowledgements, Author Contributions, Competing Interests and References were already present and correctly ordered; the main text has no tables.
  - **Figure legends moved.** The six `figure[H]` blocks were relocated out of Results into a new `sections/figure_legends.tex` under a `\section*{Figure Legends}` heading, input from `main.tex` immediately before the bibliography. Each block moved intact (image plus caption plus `\label`), so figures still number 1 to 6 in order of appearance and every `\figref` resolves unchanged.
  - **Funding Statement split out.** The editor's list requires Funding as a section distinct from Acknowledgements. The sentence "J.G. acknowledges funding support from the Schmidt Polymath Award and the Sloan Foundation." moved from Acknowledgements into a new `\section*{Funding}` placed between Acknowledgements and Author Contributions.
  - Second-round markup applied per Rule 2: the new affiliation text and the relocated funding sentence are wrapped in `\revsecond{}` in `latex/`, and are plain text in the markup-stripped submission source, which defines neither `\rev` nor `\revsecond`.
- **Type**
  - Manuscript text (author metadata, section structure)
  - Submission package
- **Verification**
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error` on both trees: 29 pages each (down from 30, the figures no longer breaking Results across pages), 0 undefined references, 0 undefined citations.
  - Rendered section order confirmed from extracted PDF text: Introduction, Results, Discussion, Methods, Data Availability, Code Availability, Acknowledgements, Funding, Author Contributions, Competing Interests, Additional Information, Figure Legends, References.
  - Each of the six captions appears exactly once in the output, and no `Fig. ??` placeholders remain.
  - Affiliation strings verified in both rendered PDFs.
  - `Main_Manuscript_compressed.pdf` rebuilt with the established Ghostscript settings (`/ebook`, `-dCompatibilityLevel=1.5`), 3.78 MB, matching the prior file's size. The source zip is 29 files, the previous 28 plus `sections/figure_legends.tex`, with build artifacts, `figures.zip` and `.DS_Store` excluded as before.
- **Still open**
  - **Acknowledgements is now an empty heading** pending author text, since its only sentence was the funding statement. Marked `\todo{}` in `latex/` (renders red) and as a TeX comment in the clean source, which does not define `\todo`. This must be resolved or the section removed before any final build.
  - Not addressed here: whether the editor wants figure image files supplied separately from the legends. The legends currently travel with their images.
  - `Supplementary_Information*` artifacts were not rebuilt; the supplementary sources have no author or affiliation block and were untouched by this change.

---

## 2026-07-30 — Submission source reconciled with the live manuscript; clean build verified

- **Affected reviewer points**: R1-2 (abstract wording), R1-code (DOI); otherwise package-level.
- **Files changed**
  - `revision_submission/00_submit_new/Main_Manuscript_Revised_LaTeX_Source/sections/{title_abstract,introduction,results,methods}.tex`
  - `revision_submission/00_submit_new/Main_Manuscript.pdf` (regenerated)
  - `revision_submission/00_submit_new/main_difference.pdf` (new)
- **What changed**
  - Added the Zenodo DOI to the clean submission source's Code Availability, in plain text rather than `\revsecond{}` because that source is markup-stripped and does not define the macro.
  - Copied the marked-up `latex/main.pdf` into the package as `main_difference.pdf`, and regenerated `Main_Manuscript.pdf` from the clean source.
  - **Divergence discovered.** The clean submission source was not simply the live text with markup stripped. A language copy-edit pass had been applied to it on 2026-07-30 (files written 00:03 to 00:05) and never back-ported to `latex/`, leaving roughly 180 differing words across five section files.
  - Verified the divergence was stylistic only: all numeric values (statistics, percentages, sample sizes, $p$-values, $\mu$ values) and all `\citep`/`\figref`/`\ref` targets were identical between the two sources, 0 differences in both checks.
  - **One difference had consequences.** The submission abstract read "dominance by one parental community" where the R1-2 response quotes "dominance of one parental community", which is also the reviewer's own suggested wording. Left unfixed this would have been a Rule 1 violation on the most trivially checkable reviewer point. Corrected. Confirmed that the separate Results phrase "asymmetric dominance by one parental community" (Fig. 2B) is intentional and present in both sources.
  - **Rolled back 11 substantial rewrites** that lengthened the text without adding content, restoring the live wording. Largest was the Fig. 4d caption, "the fraction of Mixture drops from 53\% in Nutr$-$ to 4\% in Base and remains low at 6\% in Nutr$+$" back to "Mixture declines from 53\% to 4\% to 6\%", a 12-word reduction. Others covered the outcome-classification sentence, "Analysis using alternative metrics" back to "Alternative metric analysis", the 4-to-48-species range sentence, and single-sentence restructurings in the Introduction and Methods.
  - **Kept the minor grammar fixes** from the copy-edit: "Fisher's exact test", "in simulations", "both the mean and the spread", "above the 0.1\% threshold", "Dominance in Base medium", and a comma removal. Remaining divergence is about 59 words, every hunk a delta of 0 or +1 word.
  - Re-checked all ten `\mschange{}` response-letter quotes against the submission source after editing: 10 of 10 match, up from 9 of 10.
- **Type**
  - Manuscript text
  - Submission package
- **Verification**
  - Clean-room compile: copied the source to a scratch directory with no `.aux`, `.bbl`, or other build artifacts and ran `latexmk -pdf -interaction=nonstopmode -halt-on-error`. Result: 30 pages, 0 undefined citations, 0 undefined references. The bundle is self-contained.
  - Removed build artifacts and `.DS_Store` from the submission source directory, retaining `main.bbl` since journals frequently require it.
- **Still open**
  - The live `latex/sections/*.tex` do not carry the ~59 words of retained grammar fixes; the two sources remain slightly divergent in that direction.
  - Stale in `00_submit_new`: `Response_to_Reviewers.pdf` (still carries the WORKING DRAFT banner), `Supplementary_Information.pdf`, `Main_Manuscript_compressed.pdf`, and `Main_Manuscript_Revised_LaTeX_Source.zip`, which still contains the marked-up variant rather than the clean source of the same name.

---

## 2026-07-30 — Zenodo DOI minted and wired into Code Availability (R1-code)

- **Affected reviewer point**: second-round Reviewer 1, R1-code.
- **Files changed**
  - `latex/sections/methods.tex`
  - `latex/revision_second_round/response/reviewer1_response.tex`
  - `public_code_repo/CITATION.cff`
- **What changed**
  - An archived release of the public code repository was deposited at Zenodo (record 21696638, release v1.0.0). Verified the identifiers against the Zenodo API rather than inferring them from the record URL: version DOI `10.5281/zenodo.21696638`, concept DOI `10.5281/zenodo.21696637`.
  - Used the concept DOI in citable locations so references resolve to the latest archived version rather than pinning v1.0.0.
  - `methods.tex` Code Availability now states that the repository includes the figure-generation code, which is the substance of the reviewer's complaint, and cites the Zenodo DOI alongside the GitHub URL. New text marked `\revsecond{}`.
  - The R1-code response no longer states the deposit as future intent; it now reports the DOI as a fact, which is accurate as of this entry.
  - `CITATION.cff` gained a top-level `doi` (concept) plus an `identifiers` list carrying both concept and version DOIs with descriptions. Validated as parseable YAML.
  - Zenodo populated title, all three authors with affiliations, MIT license, and version from the existing `CITATION.cff`, so no `.zenodo.json` was required.
- **Type**
  - Manuscript text
  - Response text
  - Code-availability package
- **Verification**
  - Recompiled `latex/main.tex` (30 pages) and `latex/revision_second_round/response_letter.tex` (8 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`.
- **Still open**
  - The `CITATION.cff` commit is local; `public_code_repo` has 1 unpushed commit. The Zenodo archive was cut from the earlier pushed state, so the DOI metadata in the archived snapshot will not include this change until a later release.
  - Code Availability does not mention the Dryad data DOI (10.5061/dryad.2z34tmq0z); confirm whether the journal wants data availability stated separately.

---

## 2026-07-29 — Second-round response letter critical pass

- **Affected reviewer points**: R1-1, R1-12, R2-1, plus editorial and package-level items.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
  ...
  - `revision.rule.md`
- **What changed**
  - R1-1: the response previously reported that an OD-pH analysis had been added but never stated what it showed, leaving the reviewer's direct question ("can this be buttressed by examining the correlation directly?") unanswered. It now reports the result, endpoint OD$_{600}$ and pH strongly positively correlated in Nutr$+$ ($n=60$, $\rho=0.82$, $p=8.1 \times 10^{-16}$) and Base ($n=59$, $\rho=0.79$, $p=1.6 \times 10^{-13}$) but not Nutr$-$ ($n=60$, $\rho=0.22$, $p=0.091$), and connects it to the reviewer's proposed mechanism.
  - R1-1 also now explicitly declines the reviewer's main-text suggestion and gives the reason (the association is correlational, and the main Results already carries the stronger directional acidic-versus-alkaline test in Extended Data Fig. 7). Previously the decline was silent.
  - R1-12: trimmed the second `\mschange{}` quote, which reproduced a full paragraph of which only the opening sentence was new. Now quotes the new framing sentence and states that the existing control sentence follows unchanged.
  - R2-1: removed the duplicated 59 percent versus 20 percent at $\mu = 0.6$ statistic, which had become redundant once the same numbers were added to the main-text quote earlier in this session. The supporting paragraph now cites $\mu = 0.2$ and $\mu = 1.0$ only.
  - R2-1: tightened the opening. The correction that the quoted sentence no longer exists in the manuscript was buried mid-paragraph where it read as a hedge; it is now the second sentence. Three sentences of self-criticism were reduced to one statement of the actual problem, since Reviewer 2 raised this as a minor point.
  - Response letter: the color legend described two different colors as "blue" (`ReviewerSkyBlue` RGB 0,150,220 and `SecondRoundBlue` RGB 0,70,180). Now "light blue" and "dark blue".
  - Response letter: added a short "Response to the Editor" section. The letter is addressed to the editor and reviewers but previously contained no response to the editorial requirements. The new section is deliberately modest and carries a `\draftresponse{}` block, because the editable-source format, Reporting Summary version, and availability statements are all gated on the editorial checklist and cannot be claimed yet under Rule 1.
  - `revision.rule.md`: retired the `\statusline{}` requirement in Rule 11. It mandated an internal triage marker in every response subsection, had zero occurrences in the round-2 package, and the macro was never defined. An internal marker rendered in the reviewer-facing PDF is also the wrong place for it. Status tracking is now explicitly assigned to `second_review_action_tracker.md`. Updated the two other places in the rule file that referenced the marker (the Rule 1 blocked-point procedure and workflow steps 6 and 7) so the file is internally consistent.
- **Type**
  - Response text
  - Workflow infrastructure
- **Verification**
  - Recompiled `latex/revision_second_round/response_letter.tex` (8 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`. Word counts after the pass: Reviewer 1 1,459, Reviewer 2 736, Reviewer 3 72.
- **Still open**
  - Two `\draftresponse{}` blocks remain, R1-code and the new editorial block, and `\responsedrafttrue` is still set. Both are correctly blocked rather than overlooked.
  - R1-7 does not answer the reviewer's second sub-question (why the four-community structure rather than 1,200 independent pairs). Author decision on 2026-07-29 was to leave it.
  - Reviewer 3's file uses `\responselabel` while Reviewer 1 and 2 do not.

---

## 2026-07-29 — Cross-reference format audit; fixed non-breaking space in one supplementary figure reference

- **Affected reviewer point**: none; author-initiated consistency check.
- **Files changed**
  - `latex/supplementary_sections/figures.tex`
- **What changed**
  - Audited every Supplementary cross-reference in `sections/` and `supplementary_sections/`. The convention is consistent: `Supplementary Note~N` (7 main, 14 supplementary), `Supplementary Fig.~N` (76), `Supplementary Figs.~N--M` (11), `Supplementary Methods` (14), `Supplementary Information` (5). No competing conventions found; checked for `Note S1`, `SI Note`, `Suppl.`, `Supp. Fig`, and lowercase variants, all zero hits.
  - Non-tilde occurrences of "Supplementary Note N" are section titles and `%` comments only, which are correct as written.
  - Fixed the single exception: `figures.tex` used `Supplementary Fig.\ 34` (escaped space) where all other references use a non-breaking tilde. This was functional, not only stylistic, because the escaped space allows a line break between "Fig." and the number.
- **Type**
  - Supplementary text
- **Verification**
  - Re-grepped for escaped-space reference variants; zero remain. Recompiled `latex/supplementary.tex` (62 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`.
- **Still open**
  - `sections/results.tex` line 20 cites "Supplementary Information" for a specific value (mean species survival ratio $74 \pm 2$\%) where every comparable claim cites a specific Note or Figure. Needs the actual target identified before it can be pinned.

---

## 2026-07-29 — Removed supplementary figure grouping headers

- **Affected reviewer point**: none; author-initiated cleanup.
- **Files changed**
  - `latex/supplementary_sections/figures.tex`
- **What changed**
  - Removed all 13 `\subsubsection*{\rev{...}}` grouping headers that appeared between figure captions in the Supplementary Figures section.
  - Rationale: the headers followed a "comma-list of topics plus controls" convention that produced mechanical compound titles rather than themes. Several were inaccurate, most clearly "Assembly-history and abundance-structure controls", which spanned one assembly-history control (Supplementary Fig. 23) and three descriptive rank-abundance figures (Supplementary Figs. 24 to 26) that are not controls. The grouping scheme had also drifted, with assembly filtering appearing in two separate blocks and null-model material split across three.
  - Removed headers: Strain library, coalescence examples, and assembly filtering; Baseline gLV parameter sensitivity; Nutrient-dependent invasion resistance assays; Continuous outcome-classification metrics; Pool-size, biomass, and dominant-species controls; Isolate pH phenotypes and parental-community pH structure; Laboratory-stabilized natural sample-derived communities; Assembly-history and abundance-structure controls; Nutrient-condition coalescence matrices and time series; Assembly filtering, isolate phenotypes, and parental-overlap controls; Shared-fate, classification-boundary, and additive-null controls; Mixed-sign and coefficient-structure simulation robustness; Alternative-mechanism and passive-null controls.
  - Confirmed before removal that no `\label`, `\ref`, or `\nameref` targeted any of these headers, so no cross-references broke. Figure numbering is unaffected because the headers were unnumbered starred subsubsections.
  - A backup of the pre-edit file was kept in the session scratchpad.
- **Type**
  - Supplementary text
- **Verification**
  - Recompiled `latex/supplementary.tex` (62 pages, unchanged from before the edit) with `latexmk -pdf -f -g -interaction=nonstopmode`. All 46 supplementary figure captions still present; zero `subsubsection` occurrences remain in `figures.tex`.

---

## 2026-07-29 — Second-round R2-1 assembly-history passage integrated into the paragraph argument

- **Affected reviewer point**: second-round Reviewer 2, R2-1.
- **Files changed**
  - `latex/sections/results.tex`
  - `latex/revision_second_round/response/reviewer2_response.tex`
- **What changed**
  - Rewrote the close of the assembly-effect paragraph so the direct-assembly comparison reads as the paragraph's own next step rather than as a defensive aside appended after the conclusion.
  - Reframed the comparison in active voice and led with the manipulation: "By comparing coalescence with direct assembly of the same species pool, in which species are mixed without prior separate assembly, we found that coalescence produced substantially more Dominance in both simulations and the corresponding experimental conditions." The previous version opened with a heavy front-loaded subordinate clause ("That this signature reflects prior assembly history rather than the classification itself is supported by a direct-assembly comparison"), which stated the rebuttal purpose before the reader knew what the comparison was.
  - Replaced the closing inference with "This identifies prior assembly history, rather than the outcome classification itself, as the source of the excess origin-correlated persistence." This keeps the anti-circularity point in parallel form and drops the trailing "underlying community-level selection", so the sentence no longer routes back through the label.
  - Moved the pre-existing sentence "This supports the interpretation that species within an assembled parental community have coupled fates during coalescence" from before the comparison to after it, changing the opener to "Together, these results" so it now aggregates the selection-correlation and direct-assembly evidence. This closes the paragraph on the coupled-fates hypothesis introduced in its third sentence.
  - Dropped "The within-community selection correlation thus provides a complementary species-level readout of this coupling." Its distinctive content, that the coupling is measured at the species level and separately from the outcome classification, is carried by the relocated closer and by the new "rather than the outcome classification itself" clause.
  - Markup: converted the two `\green{}` spans to `\revsecond{}`. No `\green{}` spans remain in `results.tex`, so all R2-1 manuscript text is now in the round-appropriate color. As with the R1-12 relocation, the moved closer is marked `\revsecond{}` even though only its position and opening connective changed.
  - Updated the corresponding `\mschange{}` quote in the R2-1 response.
- **Type**
  - Manuscript text
  - Response text
- **Verification**
  - Compiled `latex/main.tex` (30 pages) and `latex/revision_second_round/response_letter.tex` (8 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`.
- **Follow-up in the same session (critical re-read of the rewritten passage)**
  - Closed the paragraph's own opening question. The paragraph opens "To understand how Dominance emerges in the model and whether it reflects community-level selection", but the first rewrite dropped every mention of the label and so answered only the first half. The closer now ends "..., the origin-correlated persistence we describe as community-level selection." This is a naming relation rather than an inference, so it answers the opening question without reintroducing the circularity R2-1 objects to.
  - Softened "This identifies prior assembly history, rather than the outcome classification itself, as the source of the excess origin-correlated persistence" to "This excess therefore reflects prior assembly history rather than the way outcomes are classified." Two reasons: "identifies ... as the source" was stronger than the Discussion's "contributes to this origin dependence" for the same claim, and stronger than the response letter's explicit statement that the wording was kept at the level the design supports; and the original contrast was not parallel, since an outcome classification is not a candidate source of persistence but a candidate source of the apparent pattern.
  - Restored "combined" in "the same combined species pool". "Same" carries the control logic (same species, different history) and "combined" states what direct assembly actually mixes, the union of both parental communities' species.
  - Added the simulation magnitudes, "59\% versus 20\% at $\mu = 0.6$ in simulations", since this comparison is now the load-bearing evidence for R2-1 and Reviewer 2 specifically referred to the substantially greater frequency. Verified against `supplementary_sections/assembly_effect.tex`. The "in simulations" qualifier prevents the numbers from being read as covering the experimental half of the same sentence.
  - Dropped and then restored the `Supplementary Fig.~23` pointer. Verified that Extended Data Fig. 8 ("Assembly history promotes Dominance in simulations") is simulation-only, so with Supplementary Fig. 23 removed the phrase "and the corresponding experimental conditions" had no figure support. Supplementary Fig. 23 is the experimental panel and is restored.
  - Simplified "This excess therefore reflects prior assembly history rather than the way outcomes are classified" to "This excess therefore reflects prior assembly history." The dropped contrast was invalid: both arms of the comparison use the identical outcome classification, so the classification cannot explain a difference between them and was never a candidate explanation for the excess. The contrast was valid in the earlier version whose subject was the Dominance signature rather than the excess, and was carried over in error when the subject changed. The anti-circularity argument is made at length in the response letter and does not need to appear in the Results.
  - Checked Extended Data figure file-to-number mapping while verifying the citation: `ED_Fig7_combined.pdf` renders as Extended Data Fig. 8 and `ED_Fig8_combined.pdf` renders as Extended Data Fig. 7. Inspected `ED_Fig7_combined.pdf` directly and confirmed it contains the assembly-history simulation panel matching the Extended Data Fig. 8 caption, so the crossed filenames are a naming offset from an earlier renumbering and not a caption/image mismatch. The same inspection confirmed the 59 percent versus 20 percent values at $\mu = 0.6$ (295/499 versus 99/500).
- **Still open**
  - `results.tex` line 62 still reads "Dominance becomes increasingly frequent, indicating community-level selection", a bare inference of the kind R2-1 objects to.
  - R2-1 row in `second_review_action_tracker.md` still reads `todo`.

---

## 2026-07-29 — Second-round R2-1 null-model control sentence clarified
...
- **Files changed**
  ...
  - `latex/revision_second_round/response/reviewer2_response.tex`
- **What changed**
  - Rewrote the null-model closing sentence from "the Dominance signature exceeds what parental abundance structure alone would produce" to "the observed bias toward Dominance is stronger than what parental abundance structure alone would produce."
  - Rationale: the previous subject was a category slip, since a signature does not exceed anything. The compared quantity is parental asymmetry, $|2\mathrm{PDI}-1|$ (observed Base mean 0.812 versus 0.503 for the abundance-weighted null and 0.552 for the shuffled null; Supplementary Note 1), so the sentence now compares a magnitude to a magnitude.
  - "Bias toward Dominance" was chosen over "bias toward one parental community" so that the closing sentence brackets the paragraph's own setup sentence, which asks whether Base-medium Dominance could be a compositional artifact. Note that the underlying null-model statistics are continuous parental asymmetry rather than categorical Dominance fractions, so "bias toward Dominance" should be read as the tendency toward the Dominance region of the similarity map, not as a comparison of category counts.
  - Retained the phrase "parental abundance structure alone", which reads as an anaphoric callback to the paragraph's own setup sentence ("To test whether Base-medium Dominance could arise from a compositional artifact of parental abundance structure").
  - The sentence still stops short of claiming that the controls support community-level selection, consistent with the R2-1 restructuring.
  - Markup: converted from `\green{}` to `\revsecond{}`. One `\green{}` span now remains in `results.tex`, the assembly-history passage.
  - Updated the corresponding `\mschange{}` quote in the R2-1 response.
- **Type**
  - Manuscript text
  - Response text
...
## 2026-07-29 — Second-round R2-1 operational-definition sentences merged
...
- **Files changed**
  ...
  - `latex/revision_second_round/response/reviewer2_response.tex`
- **What changed**
  - Merged the two Section 2.1 sentences (the Dominance-as-signature sentence and the operational-definition sentence) into one: "Throughout, Dominance is the outcome-level signature that quantifies origin-correlated persistence from compositional structure, and community-level selection is our description of that persistence rather than a claim about a specific underlying mechanism."
  - Rationale: the two sentences repeated "origin-correlated persistence" and "compositional", and forced the reader to carry a referent across a sentence boundary. The merged form keeps the two-clause split that answers R2-1, with the first clause fixing Dominance as the measurement and the second fixing community-level selection as the label for what it measures.
  - The link between the two remains terminological rather than evidential (no "evidence for" or "indicates"), so the circularity Reviewer 2 identified is not reintroduced. The mechanism disclaimer stays attached to the community-level-selection label rather than to Dominance.
  - The dropped gloss ("one parental community largely displaces the other while preserving its compositional structure") is redundant with the outcome-class definition earlier in the same paragraph and with the structure-preservation statement in the time-series paragraph; the compositional-structure point is retained as "from compositional structure".
  - Markup: the merged sentence is a single `\revsecond{}` span, replacing the previous `\rev{\revsecond{...}}` plus `\green{...}` pair. Two `\green{}` spans remain in `results.tex` (null-model controls and the assembly-history comparison) and are still pending conversion to `\revsecond{}`.
  - Updated the corresponding `\mschange{}` quote in the R2-1 response.
- **Type**
  - Manuscript text
  - Response text
...
## 2026-07-29 — Second-round R1-12 paragraph order corrected

- **Affected reviewer point**: second-round Reviewer 1, R1-12.
- **Files changed**
  - `latex/sections/results.tex`
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - The paragraph split requested by the reviewer was already present; the residual problem was ordering within the second paragraph, where the theoretical interpretation trailed after the control analysis and was therefore separated from the results it interprets.
  - Moved the theoretical-interpretation sentence to close the nutrient-results paragraph, directly after the outcome distributions and the pairwise selection-correlation result that its two clauses refer to. Reworded it to "These results match the qualitative theoretical prediction and extend it to the level of selection: weak effective interactions yield Mixture with uncorrelated species fates, whereas strong effective interactions yield Dominance with community-level selection", so that the closing sentence adds the selection-level point rather than restating the outcome direction already previewed for R1-10.
  - Reframed the alternative-explanation paragraph to open with the alternative itself ("The nutrient-dependent shift could alternatively reflect changes in parental-community properties unrelated to interaction strength") rather than with the test procedure, matching the framing the reviewer supplied.
  - Markup note: the moved sentence was original text outside any `\rev{}` span, so relocating it disturbed no first-round markup. The moved sentence and the new framing sentence are marked `\revsecond{}`; the surviving first-round control sentence keeps its `\rev{}` span with `\revsecond{therefore}` nested.
  - Replaced the R1-12 drafting note with the finalized response quoting both paragraphs.
- **Type**
  - Manuscript text
  - Response text
- **Verification**
...
## 2026-07-29 — Second-round R1-13 and R1-14 responses finalized; gray-point wording aligned across captions

- **Affected reviewer points**: second-round Reviewer 1, R1-13 and R1-14.
- **Files changed**
  - `latex/sections/results.tex`
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - R1-13: replaced the drafting note with the confirmed plotting convention. Verified against `public_code_repo/figures/fig4/make_panels.py` and `figures/fig6/make_panels.py`, which plot each event twice, once under the original parental labelling in color and once with the labels swapped in gray, and against the rendered Fig. 4 panel c, which shows gray bars in the Parental Dominance Index histograms as well as gray scatter points. The response quotes the caption sentence already present in the Figs. 4 and 6 captions.
  - R1-13 follow-on: the Figs. 1 and 5 captions described the same convention with different wording ("the symmetric counterpart of each coalescence event (community B versus A)"), which also used "community" where `writing_rules.md` section 4 requires "parental community". Both were aligned to "Gray points show the same data with parental communities A and B interchanged", without the histogram clause because those panels show points only (confirmed from `figures/fig1/make_panels.py` panel e and the Fig. 5c regression panel).
  - R1-14: replaced the drafting note with the reviewer's adopted wording, quoting the full Results sentence including the retained Spearman statistics.
- **Type**
  - Manuscript text
  - Response text
- **Verification**
  - Compiled `latex/main.tex` (30 pages) and `latex/revision_second_round/response_letter.tex` (7 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`.
- **Still open**
  - R1-12 paragraph reordering, manuscript and response, pending discussion.

---

## 2026-07-29 — Second-round R1-10 hypothesized link and expected direction made explicit

- **Affected reviewer point**: second-round Reviewer 1, R1-10.
- **Files changed**
  - `latex/sections/results.tex`
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Restored the competition half of the reviewer's suggested wording: the invasion-resistance clause now reads "potentially stronger competition or other antagonistic effects, including those mediated by environmental feedbacks, at higher nutrient levels" instead of naming environmental feedbacks alone. This states the hypothesized nutrient-to-interaction-strength link at the sentence the reviewer pointed to, rather than leaving it to the upstream "net interaction effects" clause.
  - Named the anticipated direction where the coalescence experiments are introduced: "the expected shift in coalescence outcomes, from Mixture toward Dominance as effective interactions strengthen (Fig. 3)". Previously only "expected" was marked, so a reader at that sentence still had no statement of which shift was expected; the direction appeared only later in the section and in the Fig. 4 caption.
  - Both changes are nested `\revsecond{}` inside the existing first-round `\rev{}` spans.
  - Updated the R1-10 response to quote both revised sentences in `\mschange{}`.
- **Type**
  - Manuscript text
  - Response text
- **Verification**
  - Compiled `latex/main.tex` (30 pages) and `latex/revision_second_round/response_letter.tex` (7 pages) with `latexmk -pdf -f -g -interaction=nonstopmode`.
- **Not included**
  - The R1-12 paragraph reordering (theory-consistency statement before the alternative-explanation paragraph) was deliberately left for a separate pass.

---

## 2026-07-29 — R1-10 effective-interaction wording refined

- **Affected reviewer point**: second-round Reviewer 1, R1-10.
- **Files changed**
  - `latex/sections/results.tex`
- **What changed**
  - Replaced ``how net interaction effects change'' with ``changes in net effective interaction strength'' in the sentence introducing invasion resistance as the empirical approximation.
  - Preserved the qualifier ``approximate'' and marked the replacement as a second-round revision.
- **Type**
  - Manuscript text
- **Verification**
  - Compiled `latex/main.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.

---

## 2026-07-29 — Second-round R1-11 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-11.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-11 drafting note with the author-confirmed statement that the phrase was changed to ``before classifying outcomes categorically.''
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.

---

## 2026-07-29 — Second-round R1-10 wording and response finalized
...
- **Files changed**
  ...
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Condensed the nutrient-gradient sentence to report failed-invasion frequency directly and connect it cautiously to greater invasion resistance and potentially stronger antagonistic environmental feedbacks.
  - Added only the directly requested Ratzke citations to the revised interpretive clause.
  - Replaced the R1-10 drafting note with the exact revised Results wording.
- **Type**
  - Manuscript text
  - Response text
- **Verification**
  - Compiled both `latex/main.tex` and `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.

---

## 2026-07-29 — Second-round R1-9 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-9.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-9 drafting note with the author-confirmed explanation that the error-bar reference was removed and the Fig.~3 fractions were clarified as being computed from 1,200 simulated coalescence events per value of $\mu$.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-8 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-8.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-8 drafting note with the author-confirmed statement that the repetitive uses of ``Notably'' were removed.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-7 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-7.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-7 drafting note with the author-confirmed single-sentence clarification that 200 four-community replicates, each yielding six pairwise combinations, produced 1,200 total coalescence events.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-6 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-6.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-6 drafting note with the author-confirmed statement that ``introduced'' was changed to ``studied.''
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-5 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-5.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-5 drafting note with a single concise sentence stating that ``relative-abundance'' was added to the vector descriptions in the Results, Methods, and Supplementary Methods.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-4 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-4.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-4 drafting note with the author-confirmed direct response and exact revised Introduction sentence.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-3 final author wording applied

- **Affected reviewer point**: second-round Reviewer 1, R1-3.
- **Files changed**
  - `latex/sections/introduction.tex`
  - `latex/revision_second_round/response/reviewer1_response.tex`
  - `latex/revision_second_round/second_review_action_tracker.md`
- **What changed**
  - Finalized the R1-3 synthesis sentence using two concrete conditions: limited cross-community competitive exclusion and species-specific responses outweighing source-community identity in determining persistence.
  - Synchronized the exact manuscript wording in the response letter and updated the action tracker.
- **Type**
  - Manuscript text
  - Response text
  - Revision tracking
...
## 2026-07-29 — Second-round R1-3 wording and response finalized
...
- **Files changed**
  ...
  - `latex/revision_second_round/second_review_action_tracker.md`
- **What changed**
  - Finalized the concise Introduction wording that species-level selection may shape coalescence across multiple ecological contexts.
  - Retained coexistence across source communities and species-specific responses to local conditions as the two examples.
  - Replaced the R1-3 drafting note with the author-confirmed response and exact manuscript wording.
- **Type**
  - Manuscript text
  - Response text
  - Revision tracking
- **Verification**
  - Pending main-manuscript and response-letter compilation.

---

## 2026-07-29 — Second-round R1-3 sentence condensed
...
- **Files changed**
  ...
  - `latex/revision_second_round/second_review_action_tracker.md`
- **What changed**
  - Condensed the R1-3 synthesis sentence while retaining the framing that species-level selection may shape coalescence in multiple contexts.
  - Expressed the two contexts compactly as coexistence across source communities and species-specific persistence under local environmental conditions.
- **Type**
  - Manuscript text
  - Revision tracking
- **Verification**
  - Pending main-manuscript compilation.

---

## 2026-07-29 — Second-round R1-3 interpretation toned down
...
- **Files changed**
  ...
  - `latex/revision_second_round/second_review_action_tracker.md`
- **What changed**
  - Replaced the categorical ``distinct routes'' formulation with a more cautious statement that species-level selection may shape coalescence in multiple contexts.
  - Preserved the two contexts identified by the reviewer: coexistence across source communities and persistence governed by species-specific responses rather than parental-community origin.
  - Updated the action tracker to match the revised framing.
- **Type**
  - Manuscript text
  - Revision tracking
- **Verification**
...
## 2026-07-29 — Second-round R1-2 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-2.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-2 drafting note with the author-confirmed concise response and exact revised Abstract sentence.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — Second-round R1-1 response finalized

- **Affected reviewer point**: second-round Reviewer 1, R1-1.
- **Files changed**
  - `latex/revision_second_round/response/reviewer1_response.tex`
- **What changed**
  - Replaced the R1-1 drafting note with the author-confirmed response.
  - Reported the verified Nutr$+$ lower-OD winner result and quoted the corresponding Results change.
  - Directed readers to Supplementary Note~5 for the fuller biomass-control and pH-mediated interpretation.
- **Type**
  - Response text
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
...
## 2026-07-29 — R1-15 CLS definition added to Figure 5

- **Affected reviewer point**: second-round Reviewer 1, R1-15.
- **Files changed**
  - `latex/figures/figure_main_5_predictability.pdf`
  - `latex/figures/figure_main_5_predictability.png`
  - `latex/revision_second_round/response/reviewer1_response.tex`
  - `latex/revision_second_round/second_review_action_tracker.md`
- **What changed**
  - Replaced the active Figure 5 PDF with `/Users/jysong/Downloads/PDF/Asset 77.pdf`, which defines CLS as community-level selection in panel A.
  - Regenerated the matching PNG preview from the replacement PDF.
  - Replaced the R1-15 drafting note with a concise statement that the definition was added to Fig. 5.
  - Updated the second-round action tracker to describe the completed figure change.
- **Type**
  - Main figure replacement
  - Response text
  - Revision tracking
- **Verification**
  - Confirmed that the installed PDF is byte-identical to `Asset 77.pdf` by SHA-256 checksum.
  - Compiled `latex/main.tex` and `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.

---

## 2026-07-27 — Second-round response-letter scaffold

- **Affected reviewer points**: second-round response workflow; Reviewer 1 minor comments and code availability; Reviewer 2 conceptual clarification; Reviewer 3 acknowledgement.
- **Files added**
  - `latex/revision_second_round/response_letter.tex`
  - `latex/revision_second_round/response/README.md`
  - `latex/revision_second_round/response/reviewer1_response.tex`
  - `latex/revision_second_round/response/reviewer2_response.tex`
  - `latex/revision_second_round/response/reviewer3_response.tex`
  - `latex/revision_second_round/revision_figure_folder/source.md`
- **What changed**
  - Created a self-contained, compileable second-round LaTeX response package following the first-round response structure.
  - Added reviewer-point sections mapped to the second review letter.
  - Added an explicit working-draft mode and unresolved-response guards so the scaffold cannot be silently compiled as a final reviewer-facing letter.
  - Added a local response-figure folder and provenance template.
- **Type**
  - Workflow infrastructure
  - Response-letter scaffold
- **Verification**
  - Compiled `latex/revision_second_round/response_letter.tex` successfully with `latexmk -pdf -interaction=nonstopmode -halt-on-error`.
  - Generated a six-page working-draft PDF with the draft banner active.
  - Audited unresolved `\draftresponse{...}` blocks; 16 Reviewer 1 blocks and one Reviewer 2 block remain intentionally guarded pending final scientific and manuscript integration.
  - The only build warning is the expected empty bibliography warning because the scaffold does not yet cite references.
...
## 2026-08-09 — Bibliography accuracy audit and fixes

**Scope:** all 53 cited entries in `latex/references.bib` verified against CrossRef/DataCite/publisher records (authors, year, venue, title, volume/pages, DOI) plus rendered `.bbl` output. 50/53 verbatim-correct; no duplicate keys/DOIs, no broken DOIs, no undefined citations.

**Fixes applied (in `latex/references.bib`, propagated to both `revision_submission/00_submit_new/*_LaTeX_Source/references.bib` copies; all four PDFs rebuilt):**
- `SongDryad2026` / `SongZenodo2026`: titles updated from stale pre-revision manuscript title ("Interspecies interaction strength affects...") to current title ("Interspecies interactions drive community-level selection in microbial coalescence"), matching the live Dryad (10.5061/dryad.2z34tmq0z) and Zenodo (10.5281/zenodo.21696637) deposit records.
- `Rocca2021`: pages field changed to `e00538\char45 21` so the mSystems article number renders with a true hyphen ("e00538-21") instead of the en-dash produced by the bst's page-range dashify.
- `Walton2025`: added `note = {Preprint at \url{...}}` per Nature preprint citation style; independently re-verified still unpublished (bioRxiv v1 2025-11-07, v2 2026-06-01, published: NA; Crossref subtype preprint, no is-preprint-of relation).

**Not changed:** 28 uncited bib entries retained; cosmetic venue-branding variants (PLoS vs PLOS etc.) left as-is. Top-level packaged deliverables in `00_submit_new/` (Main_Manuscript.pdf, compressed PDF, Main_difference.pdf, zips) not regenerated yet — regenerate at final packaging.
