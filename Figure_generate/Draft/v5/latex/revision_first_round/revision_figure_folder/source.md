...
- **Source**: `Figure_generate/Draft/v5/latex/revision/new review responses_2026-05-14/reviewer2_agents/R2_Q5_natural_community_preselection/figures/post_stabilization_taxonomic_distinctness.pdf`
- **Code**: `Figure_generate/Draft/v5/latex/revision/new review responses_2026-05-14/reviewer2_agents/R2_Q5_natural_community_preselection/figure_code/plot_post_stabilization_taxonomic_distinctness.py`
...
- **Code**: `Figure_generate/code/Figure_revision/R3_2_reviewer_norm_comparison/reproduce_reviewer_norm_figures.py`
- **Description**: Reviewer-style geometric null reproduction under L2 normalization. Four row subtitles identify the null constructions: A, random-restructuring null model with abundances ~ U(0, 1); B, random-restructuring null model with abundances ~ 10^U(-3, 0); C, simple additive null model with abundances ~ U(0, 1); and D, simple additive null model with abundances ~ 10^U(-3, 0). Each row shows similarity maps for `N = 2, 4, 6, 8` plus the resulting Dominance/Mixture/Restructuring fractions under the manuscript classifier. Scatter panels overlay both the reviewer's visual retention boundary (`r = 1/2`) and the manuscript classifier boundary (`r = 1/sqrt(2)`). Used in the R3-2 response to show that the reviewer's qualitative geometric effect is recovered under L2/cosine normalization, while the manuscript boundary classifies random-restructuring null models primarily as Restructuring at moderate-to-large `N`, the uniform-abundance simple additive null model primarily as Mixture, and the skewed-abundance simple additive null model as high-retention but asymmetric at low `N`.
...
- **Code**: `Figure_generate/code/Figure_revision/R1_1_OD_density/analyze_OD_density.py`
- **Description**: Per-event scatter of signed OD difference (OD_A - OD_B) versus manuscript-defined PDI = (2/pi) arctan(u/v), with PDI defined as 1 for the boundary case v=0 and u>0. Panels are shown separately for each medium, with Spearman rho and p value annotated at the bottom of each panel. Reflected gray points included for symmetry.
...
- **Description**: 2x3 composite for the R1-4 response. A realized parental richness (experiment, box+jitter, ASVs) shown by medium and grouped/colored by initial species pool size. B ASV richness per inoculated isolate, computed as realized parental ASV richness divided by inoculated species-pool size. C experimental Dominance fraction shown by medium, with bars grouped and colored by initial species pool size; the displayed chi-square p=0.69 is for the full three-category outcome distribution across pool sizes, not a binary Dominance-only test. D experimental pairwise selection correlation metric grouped by initial species pool size, using the same fate-concordance implementation as Fig. 2D. E model Dominance fraction grouped by mu x pool size. F model pairwise selection correlation metric at mu in {0.30, 0.60, 0.80}, pool sizes 4/6/9/12/24/48 (100 reps each; interaction matrices reconstructed from saved seeds). Core message: Dominance frequency is set primarily by mu / nutrient condition, not by pool size, while the same-parental-community versus cross-parental-community selection correlation gap can increase with pool size.
...
- **Description**: Two-panel export reusing the experimental A/B panels from the R1-4 pool-size analysis. Panel A shows realized parental richness by medium and initial pool size. Panel B shows ASV richness per inoculated isolate by medium and initial pool size. Generated as a standalone response figure so related richness-control responses can show the experimental richness diagnostics without reproducing the full six-panel R1-4 figure.
...
- **Description**: Three-panel Base-medium continuous-distribution figure. Panels show manuscript PDI, computed as `(2/pi) arctan(u/v)` with PDI defined as 1 for the boundary case `v=0` and `u>0`, and reflection-symmetrized for visualization; asymmetry magnitude `y`; and squared retention magnitude `r^2`. Integrated as Supplementary Fig. 10.
- **Manuscript destination**: Supplementary Fig. 10 (copied to `latex/supplementary_figs/marginal_distributions_base_only.pdf`; entry in `latex/supplementary_sections/figures.tex`).
...
- **Code**: `Figure_generate/code/Figure_revision/R2_3_continuous_similarity/analyze_continuous_similarity.py`
- **Description**: Three-by-three per-medium continuous-distribution figure. Rows show Nutr-, Base, and Nutr+; columns show manuscript PDI, computed as `(2/pi) arctan(u/v)` with PDI defined as 1 for the boundary case `v=0` and `u>0`, and reflection-symmetrized for visualization; asymmetry magnitude `y`; and squared retention magnitude `r^2`. Used as Response Fig. R2-3.
- **Manuscript destination**: The Supplementary Information uses separate one-medium panels: Supplementary Fig. 10 (Base), Supplementary Fig. 11 (Nutr-), and Supplementary Fig. 12 (Nutr+).
...
- **Manuscript destination**: Supplementary Fig. 11 (copied to `latex/supplementary_figs/marginal_distributions_nutr_minus_only.pdf`; entry in `latex/supplementary_sections/figures.tex`).
...
- **Manuscript destination**: Supplementary Fig. 12 (copied to `latex/supplementary_figs/marginal_distributions_nutr_plus_only.pdf`; entry in `latex/supplementary_sections/figures.tex`).
...
- **Manuscript destination**: Supplementary Fig. 36 (copied to `latex/supplementary_figs/invasion_fitness_supp.pdf`; entered in `latex/supplementary_sections/figures.tex`).
...
- **Description**: Active Response Fig. R3-2C and Extended Data Fig. 3 focused on Base-medium synthetic coalescence events and the coalescence-pair-specific simple additive null model motivated by the R3-2 dimensionality/artifact comment. Panel A shows the manuscript-style similarity quarter circle with coalescence-pair-specific simple additive null model points, experimental points, and arrows for five randomly selected null-to-experiment pairs. Panel B shows paired asymmetry arrows from each coalescence-pair-specific simple additive null model outcome to its corresponding experimental outcome, with auxiliary lines at 0.25, 0.5, and 0.75. Panel C shows the class-transition heatmap with coalescence-pair-specific simple additive null model class on the x-axis and experimental class on the y-axis.
...
- **Code**: `Figure_generate/code/Figure_revision/R3_1_additive_null/analyze_additive_null.py`
- **Description**: Superseded standalone panel now merged into `Fig_R3_2_additive_null_comparison.pdf`; observed outcome counts versus case-by-case coalescence-pair-specific simple additive null model outcome counts.
...
- **Code**: `Figure_generate/code/Figure_revision/R3_1_additive_null/analyze_additive_null.py`
- **Description**: Per-medium contingency heatmaps for the coalescence-pair-specific simple additive null model comparison.

## fig6_asymmetricity_space.pdf
- **Source**: `Figure_generate/code/Figure_revision/R3_1_additive_null/fig6_asymmetricity_space.pdf`
- **Code**: `Figure_generate/code/Figure_revision/R3_1_additive_null/analyze_additive_null.py`
- **Description**: Superseded standalone panel now merged into `Fig_R3_2_additive_null_comparison.pdf`; observed and coalescence-pair-specific simple additive null model outcomes compared in the same retention-asymmetry space.
...
- **Code**: `Figure_generate/code/Figure_revision/R3_1_additive_null/analyze_additive_null.py`
- **Description**: Transition diagram from coalescence-pair-specific simple additive null model classifications to observed classifications.
...
- **Description**: Simulation diagnostics for parent-vector Euclidean norms, paired-parent norm fold differences, and simple additive null model classifications across interaction strengths.
...
- **Source**: `Figure_generate/code/Figure_revision/R1_7_interaction_matrix/interaction_matrix_assembly.pdf`
- **Code**: `Figure_generate/code/Figure_revision/R1_7_interaction_matrix/plot_interaction_matrix.py`
- **Manuscript destination**: `latex/supplementary_figs/interaction_matrix_post_assembly.pdf` (Supplementary Fig. 3)
...
- **Response letter reference**: Superseded R3-4 composite. The current R3-4 response now uses the split figures `R3_4_experiment.pdf`, `R3_4_mixed_sign_higher_order.pdf`, `R3_4_simulation.pdf`, `R3_4_mean_variance_grid.pdf`, and `R3_4_mutualistic_pair_fraction.pdf` as Response Figs.~R3-4a--e; the fine pair-coupling sweep remains in Supplementary Fig.~40.
...
- **Response letter reference**: Not shown as a separate response figure after the R3-4 shortening pass; retained in Supplementary Fig. 40.
- **Description**: Fine reciprocal pair-coupling sweep using the same sampler as Response Fig. R3-4c and shown in Supplementary Fig. 40. The sweep uses `p = -1, -0.8, ..., +0.8, +1` for each `mu = 0.30, 0.60, 0.80`, with 200 pools per cell. Top strips show representative labeled analytic directed-coefficient distributions for `p=-1`, `p=0`, and `p=+1`; they are formula-derived references for the sampler, not Monte Carlo coefficient samples. Fractions in `p_axis_fine_results.json` sum to 1.000000 in all 33 cells; the minimum usable event count is 1158 out of 1200. Dominance fractions across `p=-1,-0.8,-0.6,-0.4,-0.2,0,+0.2,+0.4,+0.6,+0.8,+1`: at `mu=0.30`, 43.2/39.8/37.7/31.3/27.1/21.8/25.1/32.8/39.0/43.6/46.8%; at `mu=0.60`, 70.2/68.7/59.5/58.1/59.7/63.6/68.9/72.1/74.6/75.6/78.3%; at `mu=0.80`, 77.1/71.7/67.8/68.3/64.9/72.3/75.8/78.0/80.3/77.4/78.4%.
...
- **Source**: `latex/revision/point_by_point/P3_reanalysis/R1_2_pH_dominance/make_response_figure.py`
- **Description**: Archived-style response and supplementary figure for R1-2 using the strict pH thresholds in the active manuscript and Extended Data Fig.~7 (acidic pH $<6.5$, alkaline pH $>7.5$; intermediate-pH pairings excluded). The first two panels show stacked Dominance / Mixture / Restructuring outcome fractions in strict acidic--alkaline versus strict same-pH pairings separately for Base and Nutr$+$; Fisher tests compare Dominance against pooled non-Dominance outcomes. The right panel summarizes acidic-parent winner direction within strict acidic--alkaline pairings. The same PDF is written to `latex/supplementary_figs/Fig_R1_2_acidalk_per_medium.pdf` and included as Supplementary Fig.~46.
