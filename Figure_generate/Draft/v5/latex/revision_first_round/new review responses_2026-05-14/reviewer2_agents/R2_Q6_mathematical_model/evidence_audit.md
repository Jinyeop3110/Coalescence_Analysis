...
| pH modification may contribute to winner identity in Nutr+, but strict pH contrast does not explain the full Dominance pattern. | `sections/results.tex` pH paragraph; `sections/discussion.tex` top-down regime paragraph. | Pass |
| Strict pH-contrast analysis uses parental pairs where one community has pH `<6.5` and the other has pH `>7.5`. | `sections/results.tex` pH paragraph; `supplementary_sections/extended_data.tex` Extended Data Fig. 8 caption. | Pass |
| Acidic parental communities dominated 29/32 strict acidic--alkaline Nutr+ events (90.6%) and 23/41 Base events (56.1%). | `sections/results.tex` pH paragraph; `supplementary_sections/extended_data.tex` Extended Data Fig. 8 caption. | Pass |
