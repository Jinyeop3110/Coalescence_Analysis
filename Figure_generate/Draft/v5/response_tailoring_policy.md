# Response Tailoring Policy for `v5`
...
**Working directory:** `Figure_generate/Draft/v5/`  
**Primary response source:** `latex/revision_second_round/response_letter.tex`
...
The next response-tailoring iteration starts from the current `v5` LaTeX source and may incorporate only:

1. reviewer concerns from `latex/revision_second_round/raw_second_review_letter.txt`, `latex/revision_second_round/second_review_action_tracker.md`, and the quoted comments in `latex/revision_second_round/response/*.tex`;
...
Minimal-change rule: preserve the current `v5` revision shape as much as possible. Prefer local response edits over restructuring, concise clarifications over new paragraphs, and supplementary routing over main-text expansion when the issue does not require main-text treatment.
...
Every substantive manuscript or supplementary text change should be wrapped in the correct round-specific revision macro while the rebuttal revision is under review. Preserve first-round markup as `\rev{...}`. Mark second-round manuscript and supplementary changes as `\revsecond{...}`. Both macros are defined in `latex/main.tex` and `latex/supplementary.tex`; `\rev{...}` renders red and `\revsecond{...}` renders blue in both `main.pdf` and `supplementary.pdf`.

Every exact manuscript-bound passage quoted inside the response letter should be wrapped in `\mschange{...}` according to `latex/revision_second_round/response/README.md`.
...
- active LaTeX response source in `latex/revision_second_round/response/*.tex`;
- point-by-point memos in `latex/revision_second_round/point_by_point/`;
- figure provenance in `latex/revision_second_round/revision_figure_folder/source.md` and `latex/supplementary_figs/file_source.md`;
...
The canonical response-letter style rules are in `latex/revision_second_round/response/README.md`. This policy follows that file when there is any conflict.
...
Response examples in `latex/revision_second_round/review_response_exsamples/` are style references only. They are not evidence sources for this paper.
...
- spell out `community-level selection`; use `CLS` only after defining it in the same section, caption, or response unit;
...
1. compile the response letter from `latex/revision_second_round/`;
...
grep -R -n "TODO\\|NEEDS\\|Status:\\|Confidence:" latex/sections latex/supplementary_sections latex/revision_second_round/response
grep -R -n "—\\|–\\|‘\\|’\\|“\\|”" latex/sections latex/supplementary_sections latex/revision_second_round/response
grep -R -n "Mixing\\|parent communities\\|parent community\\|consumer-resource" latex/sections latex/supplementary_sections latex/revision_second_round/response
```
...
- whether `\rev{}`, `\revsecond{}`, and `\mschange{}` are used correctly;
...
5. wrap first-round manuscript/supplement changes in `\rev{...}` and second-round manuscript/supplement changes in `\revsecond{...}`;
