...
- **Status**: [x] RESOLVED -- mean markers clarified
- **Reviewer asks**: Are means missing from ED Fig. 5C?
- **Finding**: Means are present in panel c. Visual inspection of `figures/extended_data/ED_Fig5_combined.pdf` confirms that all three panels (a, b, c) contain:
  - Small scattered dots (individual per-event correlations, 100 stratified samples displayed)
...
- **Caption states**: "Individual dots show per-event correlations (100 stratified samples displayed); squares with error bars show mean +/- s.e.m."
- **Conclusion**: The response now says the means are present as square markers with error bars and that Extended Data Fig. 5 was revised so these markers are easier to see.
