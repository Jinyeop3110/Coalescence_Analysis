...
\label{eq:glv}
\end{equation}
where $n_i$ is the abundance of species $i$, $r_i$ is its intrinsic growth rate, $S$ is the total number of species, and $\alpha_{ij}$ is the competition coefficient describing how strongly species $j$ inhibits the growth of species $i$ (with self-regulation $\alpha_{ii} = 1$). To isolate the role of interaction strength, we set all growth rates $r_i = 1$. Off-diagonal competition coefficients ($i \neq j$) were drawn randomly from a uniform distribution $U[0, 2\mu]$ with mean $\mu$, which controls the average strength of interspecific competition.
