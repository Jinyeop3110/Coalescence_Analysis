...
### R3-3 richness/geometric-effect supplement clarification
...
- **What changed**
  - Tightened the R3-3 response wording so the experimental claim explicitly refers to the observed experimental Dominance increase and the final sentence points to the simulated Dominance transition.
  - Corrected the experimental reference from Response Fig.~R3-2A to Response Fig.~R3-2C for the raw-vector/additive-null analysis.
  - Simplified the norm-fold-difference caption language in Response Figs.~R3-2C/R3-3B and Supplementary Fig.~39.
  - Expanded Supplementary Note~7 to state the low-richness/norm-imbalance implication explicitly: the effect is real under simple additive nulls, but is marginal in experiment and quantitatively insufficient to explain the simulated Dominance transition.
  - Follow-up terminology/logic pass: replaced simulation `sub-community` wording with `parental community` in the R3-3 response and Supplementary Fig.~39 caption, sharpened the experimental claim to the nutrient-dependent Dominance increase, and replaced `statistical/geometric` shorthand with `statistical or geometric`.

- **Type**
  - Response-letter text cleanup
  - Supplementary text clarification
  - Caption synchronization

### R3-5 interaction-strength terminology cleanup
...
- **What changed**
  - Rewrote the R3-5 response to distinguish the baseline gLV parameter, where $\mu$ controls non-negative growth-inhibiting competition coefficients, from the broader manuscript-level term `interaction strength`.
  - Removed awkward phrasing ("used prevalently", "support this scope") and clarified that `competition strength` is accurate for the baseline non-negative gLV ensemble, whereas `interaction strength` is retained for the broader effective-coefficient framework and robustness analyses.

- **Type**
  - Response-letter text cleanup

### R3-4 robustness-caption math simplification
...
- **What changed**
  - Simplified formula-heavy captions for Response Figs.~R3-4b/c/e/f and Supplementary Figs.~40--43.
  - Replaced set notation such as `\mu \in \{...\}` with prose such as "three representative interaction strengths."
  - Removed coefficient-distribution and gLV-equation details from captions while keeping the same conceptual descriptions and pointing detailed definitions to the response text / supplementary notes.
  - Simplified the Supplementary Fig.~34 panel F caption to avoid set notation for the pool-size ablation.

- **Type**
  - Caption cleanup
  - Response/supplement synchronization
