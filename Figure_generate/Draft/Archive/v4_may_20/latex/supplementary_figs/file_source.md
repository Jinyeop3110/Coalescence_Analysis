...
- **Description**: Reciprocal pair-coupling robustness analysis. Reciprocal coefficient structure is varied from antisymmetric exploitation (`p < 0`, one coefficient in a converted pair is negative) through the iid competitive baseline (`p = 0`) to symmetric competition (`p > 0`, both converted coefficients are positive and equal). Added together as Supplementary Fig. 41 for the Reviewer 3 interaction-structure response.
