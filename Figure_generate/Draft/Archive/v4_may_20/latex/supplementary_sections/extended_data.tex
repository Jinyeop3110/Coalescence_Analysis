...
\caption{\textbf{Extended Data Fig.~3. Testing whether skewed parental abundance distributions explain parental asymmetry.} Scatter plots show 100 randomly sampled data points with mean $\pm$ s.e.m. (squares with error bars) for experimental parental asymmetry, $|2\mathrm{PDI}-1|$, compared against two null models: (1) abundance-weighted random selection, where species survival probability is proportional to their abundance in the combined parental pool, and (2) shuffled abundance, where abundances are randomly permuted among species within each parental community before neutral mixing. Both null models produce significantly lower parental asymmetry than experimental data (Mann-Whitney U test, $p < 0.001$ for both comparisons), indicating that skewed abundance distributions alone do not fully account for the observed asymmetric outcomes.}
\label{fig:ed3}
\end{figure}
