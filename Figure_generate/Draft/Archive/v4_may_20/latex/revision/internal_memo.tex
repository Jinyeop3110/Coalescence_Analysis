...
\question{A pure-pH rule (``the more acidic parent wins'') predicts the Nutr$+$ Dominance direction well (R1-2: acidic parent wins 38/44 acid--alk events, $p < 10^{-6}$). Does this \emph{contradict} the gLV interpretation, or is it consistent?}
...
\begin{enumerate}[itemsep=2pt]
    ...
    \item \textbf{Empirical check on the winner-direction axis (Fig.~Q3, panel~a).} In acid-alk Dominance events the acidic parent wins $30/35$ in Nutr$+$ ($85.7\%$, binomial $p = 2.2 \times 10^{-5}$) and $19/27$ in Base ($70.4\%$, $p = 0.052$); in Nutr$-$ there are no acid-alk pairs by construction. These acid-win fractions exceed the gLV-implicit $50\%$ baseline, so the pH rule has real signal where gLV is silent. This is not a contradiction of gLV; it is a free axis of prediction that the baseline gLV does not use.

    \item \textbf{Empirical check on the class-frequency axis (Fig.~Q3, panel~b).} Observed Dominance fraction per medium is $38.9\%$ (Nutr$-$), $65.0\%$ (Base), $75.6\%$ (Nutr$+$); the comp-regime gLV at the nearest simulated $\mu$ values ($\mu = 0.3 / 0.6 / 0.8$, the closest set available to us without re-running; true calibrated $\mu$ is $\sim 0.5/0.7/0.9$) gives $22.2\% / 59.7\% / 73.0\%$. Base and Nutr$+$ match within a few percentage points; the Nutr$-$ mismatch is explained by the $\mu = 0.3$ comparison point being below the calibrated $\mu \approx 0.5$. The pH rule has nothing to say about this axis.
    ...
    (ii)~The pH rule is trivially calibrated against the same R1-2 acid-wins rate it is evaluated on in Nutr$+$, so its in-sample accuracy is not a legitimate test. The legitimate content of the rule is that acid-vs-alk pH is a categorical predictor of winner direction, which is an empirical fact recoverable from the pH data, not from gLV.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{internal_Q3_pH_rule_vs_gLV.pdf}
    \caption*{\textbf{Fig.~Q3.} \textbf{The pH rule and gLV predict on orthogonal
    axes, so pH-dependent winner identity does not contradict the gLV class-frequency result.} \textbf{(a)} pH-rule axis (winner direction): acid parent wins in
    $30/35$ ($86\%$) of acid-alk Dominance events in Nutr$+$ and $19/27$
    ($70\%$) in Base, both exceeding the gLV-implicit $50\%$ baseline
    (dashed). Nutr$-$ has no acid-alk pairs. Error bars: Wilson $95\%$
    CI; $p$ values: binomial test vs.\ $50\%$. \textbf{(b)} gLV axis
    (Dominance class frequency): observed (solid) vs.\ comp-regime gLV
    prediction at the nearest simulated $\mu$ (hatched). Base and
    Nutr$+$ match within a few percentage points; Nutr$-$ undershoots
    because $\mu = 0.3$ is below the calibrated $\mu \approx 0.5$.
    Generating script:
    \texttt{Figure\_generate/code/Figure\_revision/Q3\_pH\_rule\_vs\_gLV/analyze\_pH\_rule\_vs\_gLV.py}.}
\end{figure}
...
\textbf{3. Acidic-parent bias in Nutr$+$ Dominance events} \newline (R1-2; acid wins 38/44 acid--alk events, $p < 10^{-6}$; Extended Data Fig.~8) &
...
(b)~\emph{pH$\Delta$--PDI slope asymmetry across media} (Extended Data Fig.~8: Base n.s., Nutr$+$ $p = 0.002$). Subsumed under item~3: baseline has no pH axis, so the scale-asymmetry is a finer prediction on the pH-rule axis rather than an independent failure.
