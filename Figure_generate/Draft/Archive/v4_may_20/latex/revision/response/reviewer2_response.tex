...
Within the available data, we asked whether stabilized communities still carried source-specific taxonomic signal. Using same-source biological replicates as the empirical retention baseline, different-source parental communities remained substantially less similar at the ASV level (Jaccard similarity 1.7- to 4.1-fold lower than same-source replicates across the three media; Response Fig.~R2-5b), with a comparable pattern at the Genus level (Response Fig.~R2-5e). We acknowledge that distinguishability between sources does not rule out the Goldford-style scenario in which each source converges to its own source-specific attractor during stabilization; only a pre-enrichment baseline could test that directly, and we do not have one. The data do, however, exclude the stronger version of pre-selection in which all sources collapse to a single shared taxonomic endpoint. Consistent with this, coalesced communities also did not collapse to a shared within-medium endpoint, and each remained closer to its own parental pair than to unrelated same-medium parents (Response Fig.~R2-5b,c,e,f).
...
We also revised Supplementary Methods, ``Lotka--Volterra Simulations,'' to define $\alpha_{ij}$ as an effective per-capita inhibitory term and to state that the baseline $\alpha_{ij}\geq 0$ model captures competitive growth inhibition rather than facilitation.

\todo{\textbf{For Jeff/Jiliang:} the R2-6 response above states that $\mu$ controls both the mean \emph{and} variance of $\alpha_{ij}$ (because $U[0,2\mu]$ couples them), but the main text still calls $\mu$ ``\textbf{mean interaction strength}'' (\texttt{results.tex:53} [Fig.~2 caption], \texttt{methods.tex:46}, \texttt{simulations.tex:9}, \texttt{figures.tex:74}, \texttt{extended\_data.tex:46}). \textbf{Should we rename ``mean interaction strength'' $\to$ ``interaction strength'' (or ``interaction intensity'') throughout the main text?}}
...
\mschange{``Within the baseline competitive gLV ensemble, $\mu$ is a single minimal parameter that moves the off-diagonal interaction-coefficient distribution away from the no-interaction limit at zero and broadens the sampled range of interspecific effects. We interpret $\mu$ as a coarse-grained interaction-intensity axis in the model, not as a direct resource-supply parameter or as evidence that coefficient mean alone is the biological driver.''}
\mschange{``This distribution is a phenomenological ensemble of heterogeneous effective competitive effects rather than a fitted empirical distribution of biochemical mechanisms.''}
