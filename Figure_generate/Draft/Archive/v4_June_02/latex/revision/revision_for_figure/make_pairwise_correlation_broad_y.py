...
FIG2D_DIR = OUT_DIR / "Fig2D_broad_y"
...
def simulation_result(data: dict, summary: pd.DataFrame, mu: float, threshold: float = 1e-4) -> dict:
    ...
    _, p_value = stats.ttest_rel(same, cross)
...
def draw_panel(ax, result: dict, title: str | None = None, y_limits: tuple[float, float] = (-0.4, 0.4)) -> None:
    ...
    if stars != "ns":
        ...
        bracket_y = min(y_max + y_range * 0.12, ylim_high - 0.08 * (ylim_high - ylim_low))
        ...
        ax.text(np.mean(x), bracket_y, stars, ha="center", va="bottom", fontsize=9)

    ax.set_xlabel("")
    ax.set_ylabel("Pairwise selection correlation", fontsize=10)
    ax.set_xlim(-0.22, 0.90)
    ax.set_xticks(x)
...
def main() -> None:
    ...
    draw_panel(ax, sim_results[0.6], "Simulation", y_limits=(-0.8, 0.8))
    fig.tight_layout()
    save_figure(fig, "simulation_pairwise_correlation_same_style", FIG2D_DIR)
    plt.close(fig)

    fig, ax = plt.subplots(1, 1, figsize=PANEL_FIGSIZE)
    draw_panel(ax, exp_results["MN"], "Experiment", y_limits=(-0.45, 0.6))
    fig.tight_layout()
    ...
    draw_panel(axes[0], sim_results[0.6], y_limits=(-0.8, 0.8))
    draw_panel(axes[1], exp_results["MN"], y_limits=(-0.45, 0.6))
    axes[1].set_ylabel("")
    ...
    sim_y_limits = {0.3: (-0.4, 0.4), 0.6: (-0.8, 0.8), 0.8: (-0.6, 0.55)}
