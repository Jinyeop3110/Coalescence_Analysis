...
- **Description**: Active Response Fig. R3-2C and Extended Data Fig. 3 focused on Base-medium synthetic coalescence events and the event-matched raw-count additive null. Panel A shows the manuscript-style similarity quarter circle with raw-count additive-null points, observed coalesced-community points, and arrows for five randomly selected event-matched null-to-observation pairs. Panel B shows paired shifts in direction-independent parental asymmetry, y = |2PDI - 1|, from each raw-count null to its corresponding observed outcome, with the y = 0.5 asymmetry boundary and the paired Wilcoxon summary. Panel C shows the class-transition heatmap with additive-null class on the x-axis and observed class on the y-axis.
