...
| pH modification provides a mechanistic explanation for winner identity in Nutr+. | `sections/results.tex` line 102; `sections/discussion.tex` line 8. | Pass |
...
| Acidic parental communities won 38/44 acid-alkaline Nutr+ events, 86.4%, binomial `p = 9.4 x 10^{-7}`. | `sections/results.tex` line 102; Supplementary Fig. 34 caption at `supplementary_sections/figures.tex` line 424. | Pass |
