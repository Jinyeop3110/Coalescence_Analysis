...
### What They Want
A direct test of the pH modification hypothesis: if pH is the key mechanism, then mixing an acidic community with an alkaline one should create a strong selective filter, making Dominance more likely.
...
2. **Within Acid-Alk pairs, does the acidic parent win?** Signed PDI = +PDI if the acidic parent was the winner (u > v for acidic side), −PDI otherwise. Per-medium binomial test (acid wins vs 50%) and one-sample t-test (mean signed PDI vs 0).
...
- Base: 63.6% (28/44), binomial p = 0.10; signed PDI = +0.28, one-sample t p = 0.030
- Nutr+: **86.4% (38/44), binomial p = 9.4×10⁻⁷; signed PDI = +0.64, one-sample t p = 2.4×10⁻⁸**
...
The sharp test is directional: within Acid-Alk pairs in Nutr+, the acidic parent is the winner in 38/44 = 86% of events, which is a clean, well-powered confirmation of the pH-modification mechanism (acidifier-driven top-down control under high-nutrient conditions). The signal is weaker in Base, consistent with the more collective nature of Base coalescence (see R1-3).

### Figures Generated
- `Fig_R1_2_acidalk_per_medium.pdf` — 3 panels: (1) Base stacked class fractions by pair type, (2) Nutr+ stacked class fractions by pair type, (3) signed PDI for Acid-Alk pairs in Base and Nutr+.
- Previous figures `Fig_R1_2a/b/c` are deprecated and were moved to `revision/deprecated/R1_2_obsolete/`.

### Changes to Manuscript (proposed, not yet integrated)
Add a short paragraph to Results §2.5 (after the ED Fig. 8 discussion):

> "Whereas pH mismatch alone does not significantly increase Dominance frequency, within acid–alk pairs the acidic parent is systematically the winner in Nutr+ (38/44 events, p < 10⁻⁶; Supplementary Fig. [X]), directly supporting acidifier-driven top-down control as the mechanism."
