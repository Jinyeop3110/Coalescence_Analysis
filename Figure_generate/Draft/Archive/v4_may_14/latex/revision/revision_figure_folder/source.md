...
- **Code**: `Figure_generate/code/Figure_revision/R1_1_OD_density/analyze_OD_density.py`
- **Description**: Within-community pairwise selection correlation vs parental-community OD in a 3x2 grid (rows = Nutr-, Base, Nutr+). Column 1: parental communities are grouped by low/mid/high within-medium OD tertile; bars show within-community pairwise selection correlation with bootstrap 95% CIs, and a fixed dashed gray reference line shows the medium-level mean cross-community correlation with a bootstrap CI band. Column 2: per-event parental-community observations plotted by OD and within-community pairwise selection correlation, with low/mid/high OD boundary lines and region labels, Spearman rho, and p value.
...
- **Description**: Three-panel figure for R1-2, per medium (Base and Nutr+, since LN has no acidic parental communities). Panels 1-2: stacked outcome-class fractions (Dominance / Mixture / Restructuring) for the three pH-pair types ordered as Acid-Alk, Acid-Acid, Alk-Alk, with a 2-way Fisher test annotation comparing Acid-Alk vs pooled same-pH. Panel 3: signed PDI for Acid-Alk pairs (positive = acidic parental community won), with per-medium one-sample t-test and acid-wins fraction. Previous `Fig_R1_2a/b/c` figures were superseded and moved to `deprecated/R1_2_obsolete/`.
...
- **Description**: 2x3 composite for the R1-4 response, five panels filled (last slot intentionally empty). A realized parental richness (experiment, box+jitter, ASVs) shown by medium and grouped/colored by initial species pool size. B experimental Dominance fraction shown by medium, with bars grouped and colored by initial species pool size (chi-square p=0.69 pooled across media). C experimental pairwise species selection grouped by initial species pool size, same metric as the model panel below, computed on the coalescence event data. D model pairwise species selection (same solid / cross dashed) at mu in {0.30, 0.60, 0.80}, pool sizes 4/6/9/12/24/48 (100 reps each; interaction matrices reconstructed from saved seeds). E model Dominance fraction grouped by mu x pool size. Core message: community-level selection strength (Dominance frequency and the same-minus-cross pairwise-selection gap) is set by mu / nutrient condition, not by pool size. Terminology: "pairwise species selection" matches the manuscript (Fig. 2D); earlier revisions used the phrase "co-persistence" for the same quantity.
...
- **Description**: Two-panel export reusing the experimental A/B panels from the R1-4 pool-size analysis. Panel A shows realized parental richness by medium and initial pool size. Panel B shows parental ASV retention ratio by medium and initial pool size. Generated as a standalone response figure so R3-3 can show richness and retention without reproducing the full six-panel R1-4 figure.
...
- **Description**: Interaction-matrix visualization showing block structure after community assembly.
...
- **Description**: Three-panel per-medium continuous-distribution figure. Panels show manuscript PDI, computed as `(2/pi) arctan(u/v)` and reflection-symmetrized for visualization; asymmetry magnitude `y`; and squared retention magnitude `r^2`. Used as Response Fig. R2-3.
- **Manuscript destination**: Supplementary Fig. 29 (copied to `latex/supplementary_figs/marginal_distributions_by_medium.pdf`; entry in `latex/supplementary_sections/figures.tex`).
...
- **Manuscript destination**: Supplementary Fig. 30 (copied to `latex/supplementary_figs/boundary_sensitivity_by_medium.pdf`; entry in `latex/supplementary_sections/figures.tex`).
...
- **Source**: `Figure_generate/code/Figure_revision/R1_7_interaction_matrix/interaction_matrix_assembly.pdf`
- **Code**: `Figure_generate/code/Figure_revision/R1_7_interaction_matrix/analyze_interaction_matrix.py`
...
- **Description**: Post-assembly interaction matrix showing block structure. Within-community coefficients (mean 0.389) significantly lower than between-community coefficients (mean 0.500; Mann-Whitney U, p < 0.001). Integrated into manuscript 2026-04-21 as Supp. Fig. 27, with cross-reference added to Results §2.2.
...
- **Used by**: `latex/revision/internal_memo.tex` (Figs. Q5-gLV, Q5-pH, Q5-hybrid in \S Q5). Internal-memo only.
...
- **Response letter reference**: Superseded R3-4 composite. The current R3-4 response now uses the split figures `R3_4_experiment.pdf`, `R3_4_mixed_sign_higher_order.pdf`, `R3_4_simulation.pdf`, and `R3_4_pair_coupling_fine.pdf` as Response Figs.~R3-4a--d.
