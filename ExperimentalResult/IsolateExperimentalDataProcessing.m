...
LN_O=Merge_ISO_ODANDGC(LN_ISO_GC,Time)
MN_O=Merge_ISO_ODANDGC(MN_ISO_GC,Time)
HN_O=Merge_ISO_ODANDGC(HN_ISO_GC,Time)

T = table;
T = [T array2table(LN_O) T array2table(MN_O) T array2table(HN_O)]
filename = 'Postprocessed/Metadata_Isolates.xlsx';
...
%%
function O=Merge_ISO_ODANDGC(S,Time)
    A=S.GR;
    O=[]
    k=0
    for i=1:6
        for j=1:9
            k=k+1
            ODt=A{i,j};
            [O(k,1) O(k,2) O(k,3)]=GRcalculate_500(ODt,Time);
            O(k,4)=max(ODt);
        end
    end
end
