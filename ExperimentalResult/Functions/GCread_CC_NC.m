...
function [Time, Temperature, GC_struct]=GCread_CC_NC(fpath,OD_correction_function_path)

load(OD_correction_function_path)
filename=fpath;
GC_struct=struct;
GC_struct.LN=cell(15,2);
GC_struct.MN=cell(15,2);
GC_struct.HN=cell(15,2);
...
for ii=1:1
    ...
    for i=1:15
        ...
        GC_struct.LN{i,1}=y{i_,j_};
        GC_struct.LN{i,2}=y{i_,j_+2};
    ...
    for i=1:15
        ...
        GC_struct.MN{i,1}=y{i_,j_+4};
        GC_struct.MN{i,2}=y{i_,j_+6};
    ...
    for i=1:15
        ...
        GC_struct.HN{i,1}=y{i_,j_+8};
        GC_struct.HN{i,2}=y{i_,j_+10};
