...
function [Time, Temperature, GC_struct]=GCread_CC(fpath,OD_correction_function_path)

load(OD_correction_function_path)
filename=fpath;
GC_struct=struct;
GC_struct.s6=cell(14,2);
GC_struct.s12=cell(27,2);
GC_struct.s24=cell(6,2);
...
for ii=1:1
    ...
    for i=1:14
        ...
        j_=1+floor((i-1)/12);
        GC_struct.s6{i,1}=y{j_,i_};
        GC_struct.s6{i,2}=y{9-j_,13-i_};
    end
    ...
    for i=1:27
        ...
        GC_struct.s12{i,1}=y{j_,i_};
        GC_struct.s12{i,2}=y{9-j_,13-i_};
    end
    ...
    for i=1:6
        ...
        GC_struct.s24{i,1}=y{j_,i_};
        GC_struct.s24{i,2}=y{9-j_,13-i_};
    end
