...
function [Time, Temperature, GC_struct]=GCread_SC(fpath,OD_correction_function_path)

load(OD_correction_function_path)
filename=fpath;
GC_struct=struct;
GC_struct.s6=cell(9,2);
GC_struct.s12=cell(9,2);
GC_struct.s24=cell(12,2);
...
for ii=1:1
    ...
    for i=1:9
        for j=1:2
            GC_struct.s6{i,j}=y{j,i};
        end
    end

    for i=1:9
        for j=1:2
            GC_struct.s12{i,j}=y{j+3,i};
        end
    end

    for i=1:12
        for j=1:2
            GC_struct.s24{i,j}=y{j+6,i};
        end
    end
end
